...
public class QuartzScheduler implements SchedulerApi {
	...
	public void insertOrUpdateScheduleTask(int projectId, Schedule schedule) throws SchedulerException {
		...
		/**
		 * transform from server default timezone to schedule timezone e.g. server
		 * default timezone is `UTC` user set a schedule with startTime `2022-04-28
		 * 10:00:00`, timezone is `Asia/Shanghai`, api skip to transform it and save
		 * into databases directly, startTime `2022-04-28 10:00:00`, timezone is `UTC`,
		 * which actually added 8 hours, so when add job to quartz, it should recover by
		 * transform timezone
		 */
		...
		/**
		 * If the start time is less than the current time, the start time is set to the
		 * current time. We do this change to avoid misfires all triggers when update
		 * the scheduler.
		 */
		Date now = new Date();
		...
		try {
			...
			/*
			 * Instructs the Scheduler that upon a mis-fire situation, the CronTrigger wants
			 * to have it's next-fire-time updated to the next time in the schedule after
			 * the current time (taking into account any associated Calendar), but it does
			 * not want to be fired now.
			 */
