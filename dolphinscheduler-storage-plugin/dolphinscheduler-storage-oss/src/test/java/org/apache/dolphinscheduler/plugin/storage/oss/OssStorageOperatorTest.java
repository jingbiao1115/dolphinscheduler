...
public class OssStorageOperatorTest {
	...
	public void setUp() throws Exception {
		...
		doReturn(ACCESS_KEY_ID_MOCK).when(ossOperator).readOssAccessKeyID();
		doReturn(ACCESS_KEY_SECRET_MOCK).when(ossOperator).readOssAccessKeySecret();
	...
	public void testListFilesStatus() throws Exception {
		doReturn(new ListObjectsV2Result()).when(ossClientMock).listObjectsV2(Mockito.any(ListObjectsV2Request.class));
		List<StorageEntity> result = ossOperator.listFilesStatus(FULL_NAME, DEFAULT_PATH, TENANT_CODE_MOCK,
				ResourceType.FILE);
	...
	public void testListFilesStatusRecursively() throws Exception {
		...
		List<StorageEntity> result = ossOperator.listFilesStatusRecursively(FULL_NAME, DEFAULT_PATH, TENANT_CODE_MOCK,
				ResourceType.FILE);
		Assertions.assertEquals(0, result.size());
	}
}
