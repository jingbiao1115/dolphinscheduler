...
public class HdfsStorageOperator implements Closeable, StorageOperate {
	...
	public void init() throws NullPointerException {
		try {
			...
			// first get key from core-site.xml hdfs-site.xml ,if null ,then try to get from
			// properties file
	...
	/**
	 * get application url if rmHaIds contains xx, it signs not use resourcemanager
	 * otherwise: if rmHaIds is empty, single resourcemanager enabled if rmHaIds not
	 * empty: resourcemanager HA enabled
	 *
	 * @param applicationId
	 *            application id
	...
	@Override
	public List<String> vimFile(String bucketName, String hdfsFilePath, int skipLineNums, int limit)
			throws IOException {
	...
	@Override
	public boolean upload(String buckName, String srcFile, String dstPath, boolean deleteSource, boolean overwrite)
			throws IOException {
	 ...
	 *
	 * @param filePath
	 *            the path to delete, usually it is a directory.
	 * @param recursive
	 *            if path is a directory and set to true, the directory is deleted
	 *            else throws an exception. In case of a file the recursive can be
	 *            set to either true or false.
	...
	public List<StorageEntity> listFilesStatus(String path, String defaultPath, String tenantCode, ResourceType type)
			throws IOException {
		// TODO: Does listStatus truncate resultList if its size goes above certain
		// threshold (like a 1000 in S3)
	...
	@Override
	public StorageEntity getFileStatus(String path, String prefix, String tenantCode, ResourceType type)
			throws IOException {
		try {
	...
	private static final class YarnHAAdminUtils {
		...
		public static String getActiveRMName(String protocol, String rmIds) {
			...
			String yarnUrl = protocol + "%s:" + hdfsProperties.getHadoopResourceManagerHttpAddressPort()
					+ "/ws/v1/cluster/info";
		...
		public static String getRMState(String url) {
			...
			String retStr = Boolean.TRUE.equals(hdfsProperties.isHadoopSecurityAuthStartupState())
	...
	/**
	 * find alias for directories, NOT for files a directory is a path ending with
	 * "/"
	 */
	private String findDirAlias(String myStr) {
		if (!myStr.endsWith("/")) {
			// Make sure system won't crush down if someone accidentally misuse the
			// function.
			return myStr;
		}
