...
public class PostgresqlMonitor implements DatabaseMonitor {
	...
	public DatabaseMetrics getDatabaseMetrics() {
		...
		try (Connection connection = dataSource.getConnection(); Statement pstmt = connection.createStatement()) {
			...
			try (ResultSet rs3 = pstmt
					.executeQuery("select count(*) from pg_stat_activity pg where pg.state = 'active';")) {
				if (rs3.next()) {
