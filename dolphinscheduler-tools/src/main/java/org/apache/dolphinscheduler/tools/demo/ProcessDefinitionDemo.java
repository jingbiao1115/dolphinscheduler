...
public class ProcessDefinitionDemo {
	...
	public void createProcessDefinitionDemo() throws Exception {
		...
		try {
			project = Project.builder().name("demo").code(CodeGenerateUtils.getInstance().genCode()).description("")
		...
		ProxyResult dependentResult = dependentProxyResultDemo(token, projectCode, tenantCode, subProcessCode,
				switchProcessCode);
	...
	public ProxyResult clearLogDemo(String token, long projectCode, String tenantCode) {
		...
		ProxyResult ProxyResult = proxyProcessDefinitionController.createProcessDefinition(token, projectCode,
				processDefinitionLog.getName(), processDefinitionLog.getDescription(),
				processDefinitionLog.getGlobalParams(), processDefinitionLog.getLocations(),
				processDefinitionLog.getTimeout(), tenantCode, taskRelationJson, taskDefinitionJson, PARALLEL);
		return ProxyResult;
	}
