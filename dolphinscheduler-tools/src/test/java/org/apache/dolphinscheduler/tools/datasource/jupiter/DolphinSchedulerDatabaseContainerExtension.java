...
public class DolphinSchedulerDatabaseContainerExtension implements BeforeAllCallback, AfterAllCallback {
	...
	private GenericContainer<?> getDataSourceContainer(ExtensionContext context) {
		...
		DolphinSchedulerDatabaseContainer annotation = requiredTestClass
				.getAnnotation(DolphinSchedulerDatabaseContainer.class);
		if (annotation == null) {
		...
		DatabaseContainerProvider databaseContainerProvider = dataSourceContainerProviderMap
				.get(dockerImageName.getRepository());
