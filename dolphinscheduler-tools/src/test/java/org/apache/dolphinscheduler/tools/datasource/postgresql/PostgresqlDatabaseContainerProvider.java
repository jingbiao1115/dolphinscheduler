...
public class PostgresqlDatabaseContainerProvider implements DatabaseContainerProvider {
	...
	public GenericContainer<?> getContainer(DolphinSchedulerDatabaseContainer dataSourceContainer) {
		...
		GenericContainer<?> postgresqlContainer = new PostgreSQLContainer(
				DockerImageName.parse(dataSourceContainer.imageName())).withUsername("root").withPassword("root")
						.withDatabaseName("dolphinscheduler").withNetwork(NETWORK).withExposedPorts(5432);
