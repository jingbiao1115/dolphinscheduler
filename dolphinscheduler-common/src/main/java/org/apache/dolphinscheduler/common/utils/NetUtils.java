...
public class NetUtils {
	...
	protected static boolean isValidV4Address(InetAddress address) {
		...
		return (name != null && InetAddressUtils.isIPv4Address(name) && !address.isAnyLocalAddress()
	...
	protected static boolean isValidV6Address(InetAddress address) {
		...
		String name = address.getHostAddress();
		return (name != null && InetAddressUtils.isIPv6Address(name) && !address.isAnyLocalAddress()
				&& !address.isLoopbackAddress());
	}
	...
	private static List<NetworkInterface> findOuterAddressNetworkInterface(
			List<NetworkInterface> validNetworkInterfaces) {
		if (CollectionUtils.isEmpty(validNetworkInterfaces)) {
			return Collections.emptyList();
		}
