...
public final class DateUtils {
	...
	private static final DateTimeFormatter YYYY_MM_DD_HH_MM_SS = DateTimeFormatter
			.ofPattern(DateConstants.YYYY_MM_DD_HH_MM_SS);
	 ...
	 *            format
	 * @param timezone
	 *            timezone, if null, use system default timezone
	 * @return date
	 */
	...
	public static ZonedDateTime stringToZoneDateTime(@Nonnull String date) {
		...
		if (d == null) {
			throw new IllegalArgumentException(
					String.format("data: %s should be a validate data string - yyyy-MM-dd HH:mm:ss ", date));
		}
	...
	/**
	 * starting from the current time, get how many seconds are left before the
	 * target time. targetTime = baseTime + intervalSeconds
	 *
	 * @param baseTime
	 *            base time
	...
	/**
	 * transform date to target timezone date sourceTimeZoneId is system default
	 * timezone
	 */
	 ...
	 * <p>
	 * this method will return `Thu Apr 28 02:00:00 UTC 2022`
	 */
	...
	/**
	 * @param timeMillis
	 *            timeMillis like System.currentTimeMillis()
	 ...
	 *            expect formatter, like yyyy-MM-dd HH:mm:ss
