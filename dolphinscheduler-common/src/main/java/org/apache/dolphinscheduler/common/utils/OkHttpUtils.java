...
public class OkHttpUtils {
	...
	private static final OkHttpClient CLIENT = new OkHttpClient.Builder().connectTimeout(5, TimeUnit.MINUTES) // connect
																												// timeout
			.writeTimeout(5, TimeUnit.MINUTES) // write timeout
			.readTimeout(5, TimeUnit.MINUTES).build();

	public static @NonNull String get(@NonNull String url, @Nullable Map<String, String> httpHeaders,
	...
	public static @NonNull String post(@NonNull String url, @Nullable Map<String, String> httpHeaders,
			@Nullable Map<String, Object> requestParamsMap, @Nullable Map<String, Object> requestBodyMap)
			throws IOException {
		...
		if (requestBodyMap != null) {
			requestBuilder = requestBuilder.post(
					RequestBody.create(MediaType.parse("application/json"), JSONUtils.toJsonString(requestBodyMap)));
		}
	...
	public static @NonNull String demoPost(@NonNull String url, @Nullable String token,
		...
		Request request = new Request.Builder().url(url).header("token", token).addHeader("accpect", "application/json")
				.post(body).build();
