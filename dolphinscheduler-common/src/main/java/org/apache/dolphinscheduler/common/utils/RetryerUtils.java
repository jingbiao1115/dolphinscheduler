...
public class RetryerUtils {
	...
	private static Retryer<Boolean> getDefaultRetryerResultNoCheck() {
		if (defaultRetryerResultNoCheck == null) {
			defaultRetryerResultNoCheck = RetryerBuilder.<Boolean>newBuilder().retryIfException()
	...
	/**
	 * Gets default retryer. the retryer will retry 3 times if exceptions throw and
	 * wait 1 second between each retry
	 *
	 * @param checkResult
	 *            true means the callable must return true before retrying false
	 *            means that retry callable only throw exceptions
	...
	public static Retryer<Boolean> getDefaultRetryer() {
		if (defaultRetryerResultCheck == null) {
			defaultRetryerResultCheck = RetryerBuilder.<Boolean>newBuilder().retryIfResult(Boolean.FALSE::equals)
					.retryIfException()
					.withWaitStrategy(WaitStrategies.fixedWait(Constants.SLEEP_TIME_MILLIS, TimeUnit.MILLISECONDS))
					.withStopStrategy(StopStrategies.stopAfterAttempt(3)).build();
		}
	 ...
	 *            the callable
	 * @param checkResult
	 *            true means that retry callable before returning true false means
	 *            that retry callable only throw exceptions
	 ...
	 *             the retry exception
	 */
	public static Boolean retryCall(final Callable<Boolean> callable, boolean checkResult)
