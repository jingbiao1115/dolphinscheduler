...
public class AlertDao {
	...
	public void sendServerStoppedAlert(int alertGroupId, String host, String serverType) {
		ServerAlertContent serverStopAlertContent = ServerAlertContent.newBuilder().type(serverType).host(host)
				.event(AlertEvent.SERVER_DOWN).warningLevel(AlertWarnLevel.SERIOUS).build();
	...
	public void sendProcessTimeoutAlert(ProcessInstance processInstance, ProjectUser projectUser) {
		...
		ProcessAlertContent processAlertContent = ProcessAlertContent.builder()
				...
				.processState(processInstance.getState()).runTimes(processInstance.getRunTimes())
				.processStartTime(processInstance.getStartTime()).processHost(processInstance.getHost())
				.event(AlertEvent.TIME_OUT).warnLevel(AlertWarnLevel.MIDDLE).build();
	...
	public void sendTaskTimeoutAlert(ProcessInstance processInstance, TaskInstance taskInstance,
		...
		ProcessAlertContent processAlertContent = ProcessAlertContent.builder()
				...
				.owner(projectUser.getUserName()).processId(processInstance.getId())
				.processDefinitionCode(processInstance.getProcessDefinitionCode())
				.processName(processInstance.getName()).taskCode(taskInstance.getTaskCode())
				.taskName(taskInstance.getName()).taskType(taskInstance.getTaskType())
				.taskStartTime(taskInstance.getStartTime()).taskHost(taskInstance.getHost()).event(AlertEvent.TIME_OUT)
				.warnLevel(AlertWarnLevel.MIDDLE).build();
		processAlertContentList.add(processAlertContent);
		String content = JSONUtils.toJsonString(processAlertContentList);
	...
	public List<Alert> listAlerts(int processInstanceId) {
		LambdaQueryWrapper<Alert> wrapper = new LambdaQueryWrapper<Alert>().eq(Alert::getProcessInstanceId,
				processInstanceId);
	...
	public List<AlertPluginInstance> listInstanceByAlertGroupId(int alertGroupId) {
		...
		if (!Strings.isNullOrEmpty(alertInstanceIdsParam)) {
			...
			List<Integer> ids = Arrays.stream(idsArray).map(s -> Integer.parseInt(s.trim()))
