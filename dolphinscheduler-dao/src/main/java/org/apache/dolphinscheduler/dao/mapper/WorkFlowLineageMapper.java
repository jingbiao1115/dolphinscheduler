...
public interface WorkFlowLineageMapper {
	 ...
	 * @return WorkFlowLineage
	 */
	List<WorkFlowLineage> queryWorkFlowLineageByProcessDefinitionCodes(
			@Param("workFlowCodes") List<Long> workFlowCodes);

	/**
	 * queryWorkFlowLineageByCode
	 *
	 * @param processLineages
	 *            processLineages
	 * @return WorkFlowLineage list
	 */
	 ...
	 * `queryTaskDepOnTask` is a proper subset of current method
	 * `queryTaskDepOnProcess`. Which mean with the same parameter
	 * processDefinitionCode, all tasks in `queryTaskDepOnTask` are in the result of
	 * method `queryTaskDepOnProcess`.
	 *
	 * @param projectCode
	 *            Project code want to query tasks dependence
	 * @param processDefinitionCode
	...
	/**
	 * Query all tasks depend on task, only downstream task support currently(from
	 * dependent task type).
	 *
	 * In case of dependent task type, method `queryTaskDepOnTask` is a proper
	 * subset of `queryTaskDepOnProcess`. Which mean with the same
	 * processDefinitionCode, all tasks in `queryTaskDepOnTask` are in method
	...
	List<TaskMainInfo> queryTaskDepOnTask(@Param("projectCode") long projectCode,
			@Param("processDefinitionCode") long processDefinitionCode, @Param("taskCode") long taskCode);
}
