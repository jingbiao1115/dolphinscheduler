...
public class ProcessDefinitionDaoImpl extends BaseDao<ProcessDefinition, ProcessDefinitionMapper>
	...
	public PageListingResult<ProcessDefinition> listingProcessDefinition(int pageNumber, int pageSize, String searchVal,
		...
		IPage<ProcessDefinition> processDefinitions = mybatisMapper.queryDefineListPaging(page, searchVal, userId,
				projectCode);

		return PageListingResult.<ProcessDefinition>builder().totalCount(processDefinitions.getTotal())
				.currentPage(pageNumber).pageSize(pageSize).records(processDefinitions.getRecords()).build();
	}
