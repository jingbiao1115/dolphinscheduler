...
public interface TaskGroupDao extends IDao<TaskGroup> {
	...
	/**
	 * Acquire a slot for the TaskGroup which useSize should < groupSize, set the
	 * useSize = useSize + 1.
	 *
	 * @param taskGroupId
	 *            taskGroupId which shouldn't be null
	 * @return true if acquire successfully, false otherwise.
	 */
	boolean acquireTaskGroupSlot(Integer taskGroupId);

	/**
	 * Release a slot for the TaskGroup which useSize should > 0, set the useSize =
	 * useSize - 1.
	 *
	 * @param taskGroupId
	 *            taskGroupId which shouldn't be null
