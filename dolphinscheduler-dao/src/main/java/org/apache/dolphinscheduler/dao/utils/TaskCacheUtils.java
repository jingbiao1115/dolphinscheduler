...
public class TaskCacheUtils {
	...
	/**
	 * generate cache key for task instance the follow message will be used to
	 * generate cache key 2. task version 3. task is cache 4. input VarPool, from
	 * upstream task and workflow global parameters
	 * 
	 * @param taskInstance
	 *            task instance
	...
	/**
	 * generate cache key for task instance which is cache execute this key will
	 * record which cache task instance will be copied, and cache key will be used
	...
	/**
	 * get hash data of task input var pool there are two parts of task input var
	 * pool: from upstream task and workflow global parameters
	 * 
	 * @param taskInstance
	 *            task instance taskExecutionContext taskExecutionContext
	 */
	public static String getTaskInputVarPoolData(TaskInstance taskInstance, TaskExecutionContext context,
		...
		Set<String> propertyInSet = JSONUtils.toList(taskParams.get("localParams").toString(), Property.class).stream()
				.filter(property -> property.getDirect().equals(Direct.IN)).map(Property::getProp)
		...
		if (context.getPrepareParamsMap() != null) {
			...
			List<Property> globalContextVarPool = context.getPrepareParamsMap().entrySet().stream()
					.filter(entry -> !taskVarPoolSet.contains(entry.getKey())).map(Map.Entry::getValue)
					.collect(Collectors.toList());
			varPool.addAll(globalContextVarPool);
		}

		// only consider var pool value which is in propertyInSet
		varPool = varPool.stream().filter(property -> property.getDirect().equals(Direct.IN))
				.filter(property -> propertyInSet.contains(property.getProp()))
				.sorted(Comparator.comparing(Property::getProp)).collect(Collectors.toList());
	...
	/**
	 * get checksum from crc32 file of file property in varPool cache can be used if
	 * content of upstream output files are the same
	 * 
	 * @param fileProperty
	 * @param context
	 * @param storageOperate
