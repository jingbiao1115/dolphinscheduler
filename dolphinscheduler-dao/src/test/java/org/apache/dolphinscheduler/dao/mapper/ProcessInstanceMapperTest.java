...
public class ProcessInstanceMapperTest extends BaseDaoTest {
	...
	/**
	 * insert process instance with specified start time and end time,set state to
	 * SUCCESS
	 */
	...
	public void testQueryByHostAndStates() {
		...
		int[] stateArray = new int[]{TaskExecutionStatus.RUNNING_EXECUTION.getCode(),
	...
	public void testQueryProcessInstanceListPaging() {
		...
		int[] stateArray = new int[]{WorkflowExecutionStatus.RUNNING_EXECUTION.getCode(),
		...
		IPage<ProcessInstance> processInstanceIPage = processInstanceMapper.queryProcessInstanceListPaging(page,
				processDefinition.getProjectCode(), processInstance.getProcessDefinitionCode(),
				processInstance.getName(), "", stateArray, processInstance.getHost(), null, null);
	...
	public void testCountInstanceStateByUser() {
		...
		List<WorkflowInstanceStatusCountDto> workflowInstanceStatusCountDtos = processInstanceMapper
				.countWorkflowInstanceStateByProjectCodes(null, null,
	...
	public void testQueryByProcessDefineId() {
		...
		List<ProcessInstance> processInstances = processInstanceMapper
				.queryByProcessDefineCode(processInstance.getProcessDefinitionCode(), 1);
		Assertions.assertEquals(1, processInstances.size());

		processInstances = processInstanceMapper.queryByProcessDefineCode(processInstance.getProcessDefinitionCode(),
				2);
	...
	public void testQueryLastSchedulerProcess() {
		...
		ProcessInstance processInstance1 = processInstanceMapper.queryLastSchedulerProcess(
				processInstance.getProcessDefinitionCode(), null, null, processInstance.getTestFlag());
	...
	public void testQueryLastRunningProcess() {
		...
		int[] stateArray = new int[]{WorkflowExecutionStatus.RUNNING_EXECUTION.ordinal(),
				WorkflowExecutionStatus.SUBMITTED_SUCCESS.ordinal()};

		ProcessInstance processInstance1 = processInstanceMapper.queryLastRunningProcess(
				processInstance.getProcessDefinitionCode(), null, null, processInstance.getTestFlag(), stateArray);
	...
	public void testQueryLastManualProcess() {
		...
		ProcessInstance processInstance1 = processInstanceMapper.queryLastManualProcess(
				processInstance.getProcessDefinitionCode(), start, end, processInstance.getTestFlag());
		Assertions.assertEquals(processInstance1.getId(), processInstance.getId());

		start = new Date(2019 - 1900, 1 - 1, 01, 1, 0, 0);
		processInstance1 = processInstanceMapper.queryLastManualProcess(processInstance.getProcessDefinitionCode(),
				start, end, processInstance.getTestFlag());
	...
	public void testQueryTopNProcessInstance() {
		...
		List<ProcessInstance> processInstances = processInstanceMapper.queryTopNProcessInstance(2, start, end,
				WorkflowExecutionStatus.SUCCESS, 0L);
		Assertions.assertEquals(2, processInstances.size());
