...
public class DataSourceMapperTest extends BaseDaoTest {
	...
	public void testQueryDataSourceByType() {
		...
		List<DataSource> actualDataSources = dataSourceMapper.queryDataSourceByType(0, DbType.MYSQL.ordinal());
	...
	public void testListAuthorizedDataSource() {
		...
		List<DataSource> authorizedDataSource = dataSourceMapper.listAuthorizedDataSource(generalUser1.getId(),
				dataSourceIds);
	...
	public void testSelectPagingByIds() {
		...
		Assertions.assertTrue(actualDataSources.stream().map(t -> t.getId()).collect(toList()).containsAll(
				Arrays.asList(dataSource1ForUser1.getId(), dataSource2ForUser2.getId(), dataSource3ForUser1.getId())));
		...
		actualDataSources = dataSourceMapper.selectPagingByIds(page,
				Arrays.asList(dataSource1ForUser1.getId(), dataSource2ForUser2.getId()), dataSource1ForUser1.getName())
				.getRecords();
