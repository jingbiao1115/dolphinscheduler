...
public class SelectParamProps extends ParamsProps {
	...
	/**
	 * as the key name that uniquely identifies the value, it is required when the
	 * binding value is the object type
	 */
	...
	/**
	 * whether to display the selected value in the form of text when multiple
	 * selections, the default value is false
	 */
	private Boolean collapseTags;

	/**
	 * the maximum number of items that the user can select when multiple selections
	 * are made, if it is 0, there is no limit
	 */
	...
	/**
	 * whether to allow users to create new entries, it needs to be used with
	 * filterable, the default value is false
	 */
	...
	/**
	 * when multiple selection and searchable, whether to keep the current search
	 * keywords after selecting an option, the default value is false
	 */
	private Boolean reserveKeyword;

	/**
	 * press Enter in the input box to select the first match. need to be used with
	 * filterable or remote, the default value is false
	 */
	private Boolean defaultFirstOption;

	/**
	 * whether to insert a pop-up box into the body element. when there is a problem
	 * with the positioning of the pop-up box, this property can be set to false
	 */
	private Boolean popperAppendToBody;

	/**
	 * for non-searchable Select, whether to automatically pop up the option menu
	 * after the input box gets the focus, the default value is false
	 */
