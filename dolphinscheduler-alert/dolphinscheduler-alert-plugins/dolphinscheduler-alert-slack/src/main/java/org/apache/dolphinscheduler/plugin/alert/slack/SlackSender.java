...
public final class SlackSender {
	...
	/**
	 * Because the slack does not support table we can transform to specific
	 * markdown table
	 *
	 * @param content
	 *            sql data content
	...
	private String generateMarkDownTable(String content) {
		...
		stringBuilder.append(headers.stream().map(header -> generateString(header, elementLen, " "))
				.collect(Collectors.joining("|")));
		stringBuilder.append("\n");
		for (List<String> element : elements) {
			stringBuilder.append(element.stream().map(lement -> generateString("", elementLen, "-"))
					.collect(Collectors.joining("|")));
			stringBuilder.append("\n");
			stringBuilder.append(
					element.stream().map(e -> generateString(e, elementLen, " ")).collect(Collectors.joining("|")));
			stringBuilder.append("\n");
		}
