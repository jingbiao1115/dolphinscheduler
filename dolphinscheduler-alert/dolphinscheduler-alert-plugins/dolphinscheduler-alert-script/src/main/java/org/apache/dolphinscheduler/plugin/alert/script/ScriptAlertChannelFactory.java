...
public final class ScriptAlertChannelFactory implements AlertChannelFactory {
	...
	public List<PluginParams> params() {
		...
		InputParam scriptUserParam = InputParam
				...
				.setPlaceholder(AlertInputTips.CUSTOMIZED_PARAMS.getMsg()).build();
		// need check file type and file exist
		InputParam scriptPathParam = InputParam
				.newBuilder(ScriptParamsConstants.NAME_SCRIPT_PATH, ScriptParamsConstants.SCRIPT_PATH)
				.addValidate(Validate.newBuilder().setRequired(true).build())
				.setPlaceholder(AlertInputTips.SCRIPT_PATH.getMsg()).build();
		...
		RadioParam scriptTypeParams = RadioParam
				...
				.setValue(ScriptType.SHELL.getDescp()).addValidate(Validate.newBuilder().setRequired(true).build())
				.build();
