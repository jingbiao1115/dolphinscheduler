...
public class HttpAlertChannelTest {
	...
	public void testProcessSuccess() {
		...
		// HttpSender(paramsMap).send(alertData.getContent()); already test in
		// HttpSenderTest.sendTest. so we can mock
		// it
	...
	private String getParams() {
		...
		InputParam headerParams = InputParam.newBuilder("headerParams", "headerParams")
				...
				.setValue("{\"Content-Type\":\"application/json\"}").build();

		InputParam bodyParams = InputParam.newBuilder("bodyParams", "bodyParams")
				.addValidate(Validate.newBuilder().setRequired(true).build()).setValue("{\"number\":\"1234567\"}")
				.build();

		InputParam content = InputParam.newBuilder("contentField", "contentField").setValue("content")
				.addValidate(Validate.newBuilder().setRequired(true).build()).build();

		InputParam requestType = InputParam.newBuilder("requestType", "requestType").setValue("POST")
				.addValidate(Validate.newBuilder().setRequired(true).build()).build();

		InputParam timeout = InputParam.newBuilder("timeout", "timeout").setValue(120)
				.addValidate(Validate.newBuilder().setRequired(true).build()).build();
