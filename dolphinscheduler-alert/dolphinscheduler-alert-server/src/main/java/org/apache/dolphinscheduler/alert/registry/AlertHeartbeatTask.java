...
public class AlertHeartbeatTask extends BaseHeartBeatTask<AlertServerHeartBeat> {
	...
	public AlertHeartbeatTask(AlertConfig alertConfig, MetricsProvider metricsProvider, RegistryClient registryClient) {
		...
		this.heartBeatPath = RegistryNodeType.ALERT_SERVER.getRegistryPath() + "/"
	...
	public AlertServerHeartBeat getHeartBeat() {
		...
		return AlertServerHeartBeat.builder().processId(processId).startupTime(startupTime)
				...
				.jvmMemoryUsage(systemMetrics.getJvmMemoryUsedPercentage()).serverStatus(ServerStatus.NORMAL)
				.host(NetUtils.getHost()).port(alertConfig.getPort()).build();
	}
