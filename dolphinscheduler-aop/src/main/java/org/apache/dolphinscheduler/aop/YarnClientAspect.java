...
public class YarnClientAspect {
	 ...
	 *
	 * @param appContext
	 *            application context when invoking YarnClientImpl.submitApplication
	 * @param submittedAppId
	 *            the submitted application id returned by
	...
	public void registerApplicationInfo(ApplicationSubmissionContext appContext, ApplicationId submittedAppId) {
		if (appInfoFilePath != null) {
			try {
				Files.write(Paths.get(appInfoFilePath), Collections.singletonList(submittedAppId.toString()),
		...
		logger.info("YarnClientAspect[submitApplication]: current application report {}", currentApplicationReport);
	}

	/**
	 * Trigger getAppReport only when invoking getApplicationReport within
	 * submitApplication This method will invoke many times, however, the last
	 * ApplicationReport instance assigned to currentApplicationReport
	 *
	 * @param appReport
	 *            current application report when invoking getApplicationReport
	 *            within submitApplication
	 * @param appId
	 *            current application id, which is the parameter of
