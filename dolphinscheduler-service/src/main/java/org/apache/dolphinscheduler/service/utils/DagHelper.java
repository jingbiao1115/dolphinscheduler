...
public class DagHelper {
	...
	/**
	 * generate flow node relation list by task node list; Edges that are not in the
	 * task Node List will not be added to the result
	 *
	 * @param taskNodeList
	 *            taskNodeList
	...
	public static List<TaskNode> generateFlowNodeListByStartNode(List<TaskNode> taskNodeList,
			List<Long> startNodeNameList, List<Long> recoveryNodeCodeList, TaskDependType taskDependType) {
		...
		if (CollectionUtils.isEmpty(startNodeList)) {
			...
			for (Long startNodeCode : startNodeList) {
				...
				if (startNode == null) {
					...
					List<Long> visitedNodeCodeList = new ArrayList<>();
					childNodeList = getFlowNodeListPre(startNode, recoveryNodeCodeList, taskNodeList,
							visitedNodeCodeList);
				} else {
	 ...
	 * @return task node list
	 */
	private static List<TaskNode> getFlowNodeListPre(TaskNode startNode, List<Long> recoveryNodeCodeList,
			List<TaskNode> taskNodeList, List<Long> visitedNodeCodeList) {
	 ...
	 */
	public static ProcessDag generateFlowDag(List<TaskNode> totalTaskNodeList, List<Long> startNodeNameList,
			List<Long> recoveryNodeCodeList, TaskDependType depNodeType) throws Exception {
	 ...
	 * @return can submit
	 */
	public static boolean allDependsForbiddenOrEnd(TaskNode taskNode, DAG<Long, TaskNode, TaskNodeRelation> dag,
		...
		for (Long dependNodeCode : dependList) {
			...
			if (dependNode == null || completeTaskList.containsKey(dependNodeCode) || dependNode.isForbidden()
	...
	/**
	 * parse the successor nodes of previous node. this function parse the condition
	 * node to find the right branch. also check all the depends nodes forbidden or
	 * complete
	 *
	 * @return successor nodes
	 */
	public static Set<Long> parsePostNodes(Long preNodeCode, Map<Long, TaskNode> skipTaskNodeList,
			DAG<Long, TaskNode, TaskNodeRelation> dag, Map<Long, TaskInstance> completeTaskList) {
	 ...
	 */
	public static List<Long> parseConditionTask(Long nodeCode, Map<Long, TaskNode> skipTaskNodeList,
			DAG<Long, TaskNode, TaskNodeRelation> dag, Map<Long, TaskInstance> completeTaskList) {
		...
		ConditionsParameters conditionsParameters = JSONUtils.parseObject(taskNode.getConditionResult(),
	 ...
	 * @return
	 */
	public static List<Long> parseSwitchTask(Long nodeCode, Map<Long, TaskNode> skipTaskNodeList,
			DAG<Long, TaskNode, TaskNodeRelation> dag, Map<Long, TaskInstance> completeTaskList) {
		List<Long> conditionTaskList = new ArrayList<>();
		TaskNode taskNode = dag.getNode(nodeCode);
	...
	public static List<Long> skipTaskNode4Switch(TaskNode taskNode, Map<Long, TaskNode> skipTaskNodeList,
			Map<Long, TaskInstance> completeTaskList, DAG<Long, TaskNode, TaskNodeRelation> dag) {
		...
		for (SwitchResultVo info : conditionResultVoList) {
			...
			for (Long nextNode : info.getNextNode()) {
				setSwitchTaskNodeSkip(nextNode, dag, completeTaskList, skipTaskNodeList, switchNeedWorkCodes);
			}
		}
	...
	private static void setSwitchTaskNodeSkip(Long skipNodeCode, DAG<Long, TaskNode, TaskNodeRelation> dag,
			Map<Long, TaskInstance> completeTaskList, Map<Long, TaskNode> skipTaskNodeList,
			Set<Long> switchNeedWorkCodes) {
		// ignore when the node that needs to be skipped exists on the branch that the
		// switch type node needs to execute
	 ...
	 */
	private static void setTaskNodeSkip(Long skipNodeCode, DAG<Long, TaskNode, TaskNodeRelation> dag,
			Map<Long, TaskInstance> completeTaskList, Map<Long, TaskNode> skipTaskNodeList) {
	...
	public static ProcessDag getProcessDag(List<TaskNode> taskNodeList,
		...
		for (ProcessTaskRelation processTaskRelation : processTaskRelations) {
			...
			if (processTaskRelation.getPreTaskCode() != 0 && taskNodeMap.containsKey(preTaskCode)
				...
				taskNodeRelations.add(new TaskNodeRelation(preNode.getCode(), postNode.getCode()));
			}
		}
		ProcessDag processDag = new ProcessDag();
		processDag.setEdges(taskNodeRelations);
	 ...
	 */
	public static boolean haveSubAfterNode(Long parentNodeCode, DAG<Long, TaskNode, TaskNodeRelation> dag,
			String filterNodeType) {
