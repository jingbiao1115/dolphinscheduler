...
public class CuringParamsServiceImpl implements CuringParamsService {
	...
	public String convertParameterPlaceholders(String val, Map<String, Property> allParamMap) {
		Map<String, String> paramMap = allParamMap.entrySet().stream()
	...
	/**
	 * here it is judged whether external expansion calculation is required and the
	...
	public String curingGlobalParams(Integer processInstanceId, Map<String, String> globalParamMap,
			List<Property> globalParamList, CommandType commandType, Date scheduleTime, String timezone) {
		...
		// If it is a complement, a complement time needs to be passed in, according to
		// the task type
	...
	public Map<String, Property> paramParsingPreparation(@NonNull TaskInstance taskInstance,
			@NonNull AbstractParameters parameters, @NonNull ProcessInstance processInstance) {
		...
		while (iter.hasNext()) {
			...
			if (StringUtils.isNotEmpty(property.getValue())
				...
				/**
				 * local parameter refers to global parameter with the same name note: the
				 * global parameters of the process instance here are solidified parameters, and
				 * there are no variables in them.
				 */
				...
				if (timeFunctionNeedExpand(val)) {
					...
					// handle some chain parameter assign, such as `{"var1": "${var2}", "var2": 1}`
