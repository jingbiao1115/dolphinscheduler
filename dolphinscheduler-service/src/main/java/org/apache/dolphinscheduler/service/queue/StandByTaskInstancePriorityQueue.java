...
/**
 * Task instances priority queue implementation All the task instances are in
...
public class StandByTaskInstancePriorityQueue implements TaskPriorityQueue<TaskInstance> {
	 ...
	 * queue
	 */
	private final PriorityQueue<TaskInstance> queue = new PriorityQueue<>(QUEUE_MAX_SIZE,
			new TaskInstancePriorityComparator());
	 ...
	 * because this method of override interface used without considering accuracy
	 * of timeout
	 *
	 * @param timeout
	 * @param unit
	...
	// since the task instance will not contain taskInstanceId until insert into
	// database
	...
	private String getTaskInstanceIdentify(TaskInstance taskInstance) {
		return String.join(String.valueOf(taskInstance.getProcessInstanceId()),
				String.valueOf(taskInstance.getTaskCode()), String.valueOf(taskInstance.getTaskDefinitionVersion()),
				"-");
	}

	/**
	 * This comparator is used to sort task instances in the standby queue. If the
	 * TaskInstance is in the same taskGroup, then we will sort the TaskInstance by
	 * {@link TaskInstance#getTaskGroupPriority()} in the taskGroup. Otherwise, we
	 * will sort the TaskInstance by {@link TaskInstance#getTaskInstancePriority()}
	 * in the workflow.
	 */
	private static class TaskInstancePriorityComparator implements Comparator<TaskInstance> {
		...
		public int compare(TaskInstance o1, TaskInstance o2) {
			...
			int taskInstancePriorityInWorkflow = Long.compare(o1.getTaskInstancePriority().getCode(),
