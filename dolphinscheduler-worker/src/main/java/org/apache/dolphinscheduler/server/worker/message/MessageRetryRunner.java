...
public class MessageRetryRunner extends BaseDaemonThread {
	...
	private final Map<ITaskInstanceExecutionEvent.TaskInstanceExecutionEventType, TaskInstanceExecutionEventSender<ITaskInstanceExecutionEvent>> messageSenderMap = new HashMap<>();
	...
	public void run() {
		while (!ServerLifeCycleManager.isStopped()) {
			try {
				...
				Iterator<Map.Entry<Integer, List<TaskInstanceMessage>>> iterator = needToRetryMessages.entrySet()
						.iterator();
				while (iterator.hasNext()) {
					...
					try {
						for (TaskInstanceMessage taskInstanceMessage : taskInstanceMessages) {
							ITaskInstanceExecutionEvent.TaskInstanceExecutionEventType eventType = taskInstanceMessage
									.getEventType();
