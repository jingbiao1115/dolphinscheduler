...
public class TaskExecutionContextUtils {
	...
	public static String getOrCreateTenant(WorkerConfig workerConfig, TaskExecutionContext taskExecutionContext) {
		try {
			...
			// if Using distributed is true and Currently supported systems are linux,Should
			// not let it
			// automatically
	...
	public static void createTaskInstanceWorkingDirectory(TaskExecutionContext taskExecutionContext)
			throws TaskException {
		// local execute path
		String taskInstanceWorkingDirectory = FileUtils.getTaskInstanceWorkingDirectory(
				taskExecutionContext.getTenantCode(), taskExecutionContext.getProjectCode(),
	...
	public static ResourceContext downloadResourcesIfNeeded(String tenant, TaskChannel taskChannel,
			StorageOperate storageOperate, TaskExecutionContext taskExecutionContext) {
		AbstractParameters abstractParameters = taskChannel.parseParameters(ParametersNode.builder()
				.taskType(taskExecutionContext.getTaskType()).taskParams(taskExecutionContext.getTaskParams()).build());
		...
		for (ResourceInfo resourceInfo : resourceFilesList) {
			...
			ResourceContext.ResourceItem resourceItem = ResourceContext.ResourceItem.builder()
					...
					.resourceRelativePath(resourceRelativePath).resourceAbsolutePathInLocal(resourceAbsolutePathInLocal)
					.build();
