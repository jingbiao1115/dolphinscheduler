...
public abstract class WorkerTaskExecutor implements Runnable {
	...
	protected WorkerTaskExecutor(@NonNull TaskExecutionContext taskExecutionContext, @NonNull WorkerConfig workerConfig,
			@NonNull WorkerMessageSender workerMessageSender, @NonNull TaskPluginManager taskPluginManager,
			@Nullable StorageOperate storageOperate, @NonNull WorkerRegistryClient workerRegistryClient) {
	...
	public void run() {
		try {
			...
			TaskCallBack taskCallBack = TaskCallbackImpl.builder().workerMessageSender(workerMessageSender)
					.taskExecutionContext(taskExecutionContext).build();
	...
	protected void sendAlertIfNeeded() {
		...
		AlertSendRequest alertSendRequest = new AlertSendRequest(taskAlertInfo.getAlertGroupId(),
