...
public class WorkerHeartBeatTask extends BaseHeartBeatTask<WorkerHeartBeat> {
	...
	public WorkerHeartBeatTask(@NonNull WorkerConfig workerConfig, @NonNull MetricsProvider metricsProvider,
	...
	public WorkerHeartBeat getHeartBeat() {
		...
		return WorkerHeartBeat.builder().startupTime(ServerLifeCycleManager.getServerStartupTime())
				.reportTime(System.currentTimeMillis()).cpuUsage(systemMetrics.getTotalCpuUsedPercentage())
				.jvmMemoryUsage(systemMetrics.getJvmMemoryUsedPercentage())
				.memoryUsage(systemMetrics.getSystemMemoryUsedPercentage())
				.diskUsage(systemMetrics.getDiskUsedPercentage()).processId(processId)
				...
				.serverStatus(serverStatus).host(NetUtils.getHost()).port(workerConfig.getListenPort()).build();
	}
	...
	private ServerStatus getServerStatus(SystemMetrics systemMetrics, WorkerConfig workerConfig,
