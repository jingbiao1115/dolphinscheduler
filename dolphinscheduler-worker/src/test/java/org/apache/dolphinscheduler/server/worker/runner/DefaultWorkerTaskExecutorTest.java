...
public class DefaultWorkerTaskExecutorTest {
	...
	public void testDryRun() {
		TaskExecutionContext taskExecutionContext = TaskExecutionContext.builder().dryRun(Constants.DRY_RUN_FLAG_YES)
				.taskInstanceId(0).processDefineId(0).firstSubmitTime(System.currentTimeMillis()).build();
		WorkerTaskExecutor workerTaskExecutor = new DefaultWorkerTaskExecutor(taskExecutionContext, workerConfig,
				workerMessageSender, taskPluginManager, storageOperate, workerRegistryClient);
	...
	public void testErrorboundTestDataSource() {
		TaskExecutionContext taskExecutionContext = TaskExecutionContext.builder().dryRun(Constants.DRY_RUN_FLAG_NO)
				.testFlag(Constants.TEST_FLAG_YES).taskInstanceId(0).processDefineId(0)
				.firstSubmitTime(System.currentTimeMillis()).taskType("SQL")
				.taskParams(
