...
public class TaskFilesTransferUtilsTest {
	...
	void init() {
		...
		exceptTemplate = String.format("%s/%s/%d/%d_%d/%s_%d", TaskFilesTransferUtils.RESOURCE_TAG, date,
				processDefineCode, processDefineVersion, processInstanceId, taskName, taskInstanceId);
	}
	...
	void testUploadOutputFiles() throws IOException {
		...
		TaskExecutionContext taskExecutionContext = TaskExecutionContext.builder().varPool(varPool)
				.taskParams(taskParams).processInstanceId(processInstanceId).processDefineVersion(processDefineVersion)
				.processDefineCode(processDefineCode).taskInstanceId(taskInstanceId).taskName(taskName)
				.tenantCode(tenantCode).executePath(executePath.toString()).endTime(endTime).build();
		...
		String exceptFolder = String.format("%s_%s", exceptTemplate,
				folderPath.getName() + TaskFilesTransferUtils.PACK_SUFFIX);
	...
	void testDownloadUpstreamFiles() {
		...
		String varPool = "["
				+ String.format("{\"prop\":\"task1.folder\",\"direct\":\"IN\",\"type\":\"FILE\",\"value\":\"%s\"},",
						folderPath)
