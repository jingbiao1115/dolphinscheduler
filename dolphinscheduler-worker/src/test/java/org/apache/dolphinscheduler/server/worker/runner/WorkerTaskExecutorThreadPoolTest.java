...
class WorkerTaskExecutorThreadPoolTest {
	...
	public void testIsOverload() {
		...
		for (int i = 0; i < 100; i++) {
			boolean submitResult = workerTaskExecutorThreadPool
					.submitWorkerTaskExecutor(new MockWorkerTaskExecutor(() -> {
						try {
		...
		protected MockWorkerTaskExecutor(Runnable runnable) {
			super(TaskExecutionContext.builder().taskInstanceId((int) System.nanoTime()).build(), new WorkerConfig(),
					new WorkerMessageSender(), new TaskPluginManager(), new StorageOperate() {
						...
						@Override
						public boolean delete(String filePath, List<String> childrenPathArray, boolean recursive) {
							return false;
						}
						...
						@Override
						public List<String> vimFile(String tenantCode, String filePath, int skipLineNums, int limit) {
							return null;
						}
