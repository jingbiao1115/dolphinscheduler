...
public class OracleDataSourceProcessor extends AbstractDataSourceProcessor {
	...
	public BaseConnectionParam createConnectionParams(BaseDataSourceParamDTO datasourceParam) {
		...
		if (DbConnectType.ORACLE_SID.equals(oracleParam.getConnectType())) {
			address = String.format("%s%s:%s", DataSourceConstants.JDBC_ORACLE_SID, oracleParam.getHost(),
					oracleParam.getPort());
			jdbcUrl = address + ":" + oracleParam.getDatabase();
		} else {
			address = String.format("%s%s:%s", DataSourceConstants.JDBC_ORACLE_SERVICE_NAME, oracleParam.getHost(),
					oracleParam.getPort());
			jdbcUrl = address + "/" + oracleParam.getDatabase();
		}
	...
	public Connection getConnection(ConnectionParam connectionParam) throws ClassNotFoundException, SQLException {
		...
		return DriverManager.getConnection(getJdbcUrl(connectionParam), oracleConnectionParam.getUser(),
