...
/**
 * This class is used to create a proxy client which will transform local method
 * invocation to remove invocation.
 */
public class JdkDynamicRpcClientProxyFactory implements IRpcClientProxyFactory {
	...
	private static final LoadingCache<String, Map<String, Object>> proxyClientCache = CacheBuilder.newBuilder()
			...
			.expireAfterAccess(Duration.ofHours(1)).build(new CacheLoader<String, Map<String, Object>>() {
	...
	public <T> T getProxyClient(String serverHost, Class<T> clientInterface) {
		return (T) proxyClientCache.get(serverHost).computeIfAbsent(clientInterface.getName(),
