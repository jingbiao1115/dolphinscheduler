	...
	private void processReceived(final Channel channel, final Transporter transporter) {
		...
		try {
			if (methodInvoker == null) {
				...
				StandardRpcResponse iRpcResponse = StandardRpcResponse
						.fail("Cannot find the ServerMethodInvoker of " + methodIdentifier);
				TransporterHeader transporterHeader = TransporterHeader.of(transporter.getHeader().getOpaque(),
						methodIdentifier);
			...
			nettyRemotingServer.getDefaultExecutor().execute(() -> {
				...
				try {
					StandardRpcRequest standardRpcRequest = JsonSerializer.deserialize(transporter.getBody(),
