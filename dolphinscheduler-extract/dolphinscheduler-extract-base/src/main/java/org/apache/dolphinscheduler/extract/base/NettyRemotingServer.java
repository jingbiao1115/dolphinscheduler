...
public class NettyRemotingServer {
	...
	public NettyRemotingServer(final NettyServerConfig serverConfig) {
		...
		ThreadFactory bossThreadFactory = ThreadUtils
				.newDaemonThreadFactory(serverConfig.getServerName() + "BossThread_%s");
		ThreadFactory workerThreadFactory = ThreadUtils
				.newDaemonThreadFactory(serverConfig.getServerName() + "WorkerThread_%s");
	...
	public void start() {
		if (isStarted.compareAndSet(false, true)) {
			this.serverBootstrap.group(this.bossGroup, this.workGroup).channel(NettyUtils.getServerSocketChannelClass())
	...
	private void initNettyChannel(SocketChannel ch) {
		ch.pipeline().addLast("encoder", new TransporterEncoder()).addLast("decoder", new TransporterDecoder())
