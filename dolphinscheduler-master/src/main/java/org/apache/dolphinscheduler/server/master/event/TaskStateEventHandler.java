...
public class TaskStateEventHandler implements StateEventHandler {
	...
	public boolean handleStateEvent(WorkflowExecuteRunnable workflowExecuteRunnable, StateEvent stateEvent)
		...
		Optional<TaskInstance> taskInstanceOptional = workflowExecuteRunnable
				.getTaskInstance(taskStateEvent.getTaskInstanceId());
		...
		log.info("Handle task instance state event, the current task instance state {} will be changed to {}",
		...
		if (task.getState().isFinished()
			...
			String errorMessage = String.format(
					...
					task.getName(), task.getState().name(), taskStateEvent.getStatus().name());
