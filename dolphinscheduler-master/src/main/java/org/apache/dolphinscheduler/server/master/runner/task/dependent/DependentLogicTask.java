...
public class DependentLogicTask extends BaseAsyncLogicTask<DependentParameters> {
	...
	public DependentLogicTask(TaskExecutionContext taskExecutionContext, ProjectDao projectDao,
			ProcessDefinitionDao processDefinitionDao, TaskDefinitionDao taskDefinitionDao,
			TaskInstanceDao taskInstanceDao, ProcessInstanceDao processInstanceDao,
	...
	public AsyncTaskExecuteFunction getAsyncTaskExecuteFunction() {
		return new DependentAsyncTaskExecuteFunction(taskExecutionContext, taskParameters, projectDao,
				processDefinitionDao, taskDefinitionDao, taskInstanceDao, processInstanceDao);
	}
	...
	public void pause() throws MasterTaskExecuteException {
		WorkflowExecuteRunnable workflowExecuteRunnable = processInstanceExecCacheManager
				.getByProcessInstanceId(taskExecutionContext.getProcessInstanceId());
		if (workflowExecuteRunnable == null) {
			log.error("Cannot find the WorkflowExecuteRunnable");
			return;
		}
		TaskInstance taskInstance = workflowExecuteRunnable.getTaskInstance(taskExecutionContext.getTaskInstanceId())
				.orElse(null);
