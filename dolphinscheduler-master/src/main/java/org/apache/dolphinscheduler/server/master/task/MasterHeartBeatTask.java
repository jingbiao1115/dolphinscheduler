...
public class MasterHeartBeatTask extends BaseHeartBeatTask<MasterHeartBeat> {
	...
	public MasterHeartBeatTask(@NonNull MasterConfig masterConfig, @NonNull MetricsProvider metricsProvider,
	...
	public MasterHeartBeat getHeartBeat() {
		...
		return MasterHeartBeat.builder().startupTime(ServerLifeCycleManager.getServerStartupTime())
				...
				.diskUsage(systemMetrics.getDiskUsedPercentage()).processId(processId).serverStatus(serverStatus)
				.host(NetUtils.getHost()).port(masterConfig.getListenPort()).build();
	}
