...
public class ConditionLogicTask extends BaseSyncLogicTask<DependentParameters> {
	...
	public ConditionLogicTask(TaskExecutionContext taskExecutionContext,
			ProcessInstanceExecCacheManager processInstanceExecCacheManager, TaskInstanceDao taskInstanceDao,
			ProcessInstanceDao workflowInstanceDao) throws LogicTaskInitializeException {
		// todo: we need to change the parameter in front-end, so that we can directly
		// use json to parse
	...
	public void handle() {
		...
		TaskExecutionStatus taskExecutionStatus = (conditionResult == DependResult.SUCCESS)
	...
	private DependResult calculateConditionResult() {
		final ProcessInstance processInstance = workflowInstanceDao
				.queryById(taskExecutionContext.getProcessInstanceId());
		final List<TaskInstance> taskInstances = taskInstanceDao
		...
		List<DependResult> dependResults = taskParameters.getDependTaskList().stream()
				.map(dependentTaskModel -> DependentUtils.getDependResultForRelation(dependentTaskModel.getRelation(),
						dependentTaskModel.getDependItemList().stream()
