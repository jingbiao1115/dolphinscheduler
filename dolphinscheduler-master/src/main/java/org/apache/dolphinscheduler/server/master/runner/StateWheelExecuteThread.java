...
/**
 * Check thread 1. timeout task check 2. dependent task state check 3. retry
 * task check 4. timeout process check
 */
@Component
@Slf4j
public class StateWheelExecuteThread extends BaseDaemonThread {
	...
	private void checkProcess4Timeout() {
		...
		for (Integer processInstanceId : processInstanceTimeoutCheckList) {
			try {
				...
				ProcessInstance processInstance = workflowExecuteThread.getWorkflowExecuteContext()
						.getWorkflowInstance();
	...
	private void checkTask4Timeout() {
		...
		for (TaskInstanceKey taskInstanceKey : taskInstanceTimeoutCheckList) {
			try {
				...
				Optional<TaskInstance> taskInstanceOptional = workflowExecuteThread
						.getActiveTaskInstanceByTaskCode(taskCode);
	...
	private void checkTask4Retry() {
		...
		for (TaskInstanceKey taskInstanceKey : taskInstanceRetryCheckList) {
			...
			try {
				...
				if (workflowExecuteThread == null) {
					log.warn("Task instance retry check failed, can not find workflowExecuteThread from cache manager, "
							+ "will remove this check task");
					taskInstanceRetryCheckList.remove(taskInstanceKey);
					continue;
				}
				...
				Optional<TaskInstance> taskInstanceOptional = workflowExecuteThread
						.getRetryTaskInstanceByTaskCode(taskCode);
				ProcessInstance processInstance = workflowExecuteThread.getWorkflowExecuteContext()
						.getWorkflowInstance();
				...
				TaskInstance taskInstance = taskInstanceOptional.get();
				// We check the status to avoid when we do worker failover we submit a failover
				// task, this task may be
				// resubmit by this
				// thread
	...
	private void addProcessStopEvent(ProcessInstance processInstance) {
		WorkflowStateEvent stateEvent = WorkflowStateEvent.builder().processInstanceId(processInstance.getId())
				.type(StateEventType.PROCESS_STATE_CHANGE).status(WorkflowExecutionStatus.STOP).build();
		workflowExecuteThreadPool.submitStateEvent(stateEvent);
	}

	private void addTaskRetryEvent(TaskInstance taskInstance) {
		TaskStateEvent stateEvent = TaskStateEvent.builder().processInstanceId(taskInstance.getProcessInstanceId())
				.taskCode(taskInstance.getTaskCode()).status(TaskExecutionStatus.RUNNING_EXECUTION)
				.type(StateEventType.TASK_RETRY).build();
		workflowExecuteThreadPool.submitStateEvent(stateEvent);
	}

	private void addTaskTimeoutEvent(TaskInstance taskInstance) {
		TaskStateEvent stateEvent = TaskStateEvent.builder().processInstanceId(taskInstance.getProcessInstanceId())
				.taskInstanceId(taskInstance.getId()).type(StateEventType.TASK_TIMEOUT)
				.taskCode(taskInstance.getTaskCode()).build();
		workflowExecuteThreadPool.submitStateEvent(stateEvent);
	}

	private void addProcessTimeoutEvent(ProcessInstance processInstance) {
		WorkflowStateEvent stateEvent = WorkflowStateEvent.builder().processInstanceId(processInstance.getId())
				.type(StateEventType.PROCESS_TIMEOUT).build();
		workflowExecuteThreadPool.submitStateEvent(stateEvent);
	}
