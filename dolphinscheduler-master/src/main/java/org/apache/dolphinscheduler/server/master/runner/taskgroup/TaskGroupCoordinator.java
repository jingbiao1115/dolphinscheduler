 ...
 * slot is used to limit the number of {@link TaskInstance} that can be run at
 * the same time.
 * <p>
 * The {@link TaskGroupQueue} is used to represent the task group slot. When a
 * {@link TaskGroupQueue} which inQueue is YES means the {@link TaskGroupQueue}
 * is using by a {@link TaskInstance}.
 * <p>
 * When the {@link TaskInstance} need to use task group, we should
 * use @{@link TaskGroupCoordinator#acquireTaskGroupSlot(TaskInstance)} to
 * acquire the task group slot, this method doesn't block should always acquire
 * successfully, and you should directly stop dispatch the task instance. When
 * the task group slot is available, the TaskGroupCoordinator will wake up the
 * waiting {@link TaskInstance} to dispatch.
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * When the {@link TaskInstance} is finished, we should
 * use @{@link TaskGroupCoordinator#releaseTaskGroupSlot(TaskInstance)} to
 * release the task group slot.
 * 
 * <pre>
...
public class TaskGroupCoordinator extends BaseDaemonThread {
	...
	/**
	 * Make sure the TaskGroup useSize is equal to the TaskGroupQueue which status
	 * is {@link TaskGroupQueueStatus#ACQUIRE_SUCCESS} and forceStart is
	 * {@link org.apache.dolphinscheduler.common.enums.Flag#NO}.
	 */
	private void amendTaskGroupUseSize() {
		// The TaskGroup useSize should equal to the TaskGroupQueue which inQueue is YES
		// and forceStart is NO
		List<TaskGroup> taskGroups = taskGroupDao.queryAllTaskGroups();
		if (CollectionUtils.isEmpty(taskGroups)) {
			return;
		}
		for (TaskGroup taskGroup : taskGroups) {
			List<TaskGroupQueue> taskGroupQueues = taskGroupQueueDao
					.queryAcquiredTaskGroupQueueByGroupId(taskGroup.getId());
	...
	/**
	 * Make sure the TaskGroupQueue status is {@link TaskGroupQueueStatus#RELEASE}
	 * when the related {@link TaskInstance} is not exist or status is finished.
	 */
	private void amendTaskGroupQueueStatus() {
		...
		List<Integer> taskInstanceIds = taskGroupQueues.stream().map(TaskGroupQueue::getTaskId)
				.collect(Collectors.toList());
		Map<Integer, TaskInstance> taskInstanceMap = taskInstanceDao.queryByIds(taskInstanceIds).stream()
	...
	private void dealWithForceStartTaskGroupQueue() {
		...
		List<TaskGroupQueue> taskGroupQueues = taskGroupQueueDao.queryAllInQueueTaskGroupQueue().stream()
		...
		for (TaskGroupQueue taskGroupQueue : taskGroupQueues) {
			try {
				...
				// We notify first, it notify failed, the taskGroupQueue will be in queue, and
				// then we will retry it
				// next time.
	...
	private void dealWithWaitingTaskGroupQueue() {
		...
		// Find the highest priority inQueue task group queue(Which is inQueue and
		// status is Waiting and force start is
		// NO) belong to the
		// task group.
		...
		for (TaskGroup taskGroup : taskGroups) {
			...
			List<TaskGroupQueue> taskGroupQueues = taskGroupQueueDao
					.queryAllInQueueTaskGroupQueueByGroupId(taskGroup.getId()).stream()
					...
					.limit(availableSize).collect(Collectors.toList());
	...
	/**
	 * If the {@link TaskInstance#getTaskGroupId()} > 0, and the TaskGroup flag is
	 * {@link Flag#YES} then the task instance need to use task group.
	 *
	 * @param taskInstance
	 *            task instance
	 ...
	 * <p>
	 * When taskInstance want to acquire a TaskGroup slot, should call this method.
	 * If acquire successfully, will create a TaskGroupQueue in db which is in queue
	 * and status is {@link TaskGroupQueueStatus#WAIT_QUEUE}. The TaskInstance
	 * shouldn't dispatch until there exist available slot, the taskGroupCoordinator
	 * notify it.
	 *
	 * @param taskInstance
	 *            the task instance which want to acquire task group slot.
	 * @throws IllegalArgumentException
	 *             if the taskInstance is null or the used taskGroup doesn't exist.
	 */
	public void acquireTaskGroupSlot(TaskInstance taskInstance) {
		...
		// Write TaskGroupQueue in db, and then return wait TaskGroupCoordinator to
		// notify it
		// Set the taskGroupQueue status to WAIT_QUEUE and add to queue
		// The queue only contains the taskGroupQueue which status is WAIT_QUEUE or
		// ACQUIRE_SUCCESS
		Date now = new Date();
		TaskGroupQueue taskGroupQueue = TaskGroupQueue.builder().taskId(taskInstance.getId())
				.taskName(taskInstance.getName()).groupId(taskInstance.getTaskGroupId())
				.processId(taskInstance.getProcessInstanceId()).priority(taskInstance.getTaskGroupPriority())
				.inQueue(Flag.YES.getCode()).forceStart(Flag.NO.getCode()).status(TaskGroupQueueStatus.WAIT_QUEUE)
	 ...
	 * <p>
	 * When taskInstance want to release a TaskGroup slot, should call this method.
	 * The release method will move the TaskGroupQueue out queue and set status to
	 * {@link TaskGroupQueueStatus#RELEASE}. This method is idempotent, this means
	 * that if the task group slot is already released, this method will do nothing.
	 *
	 * @param taskInstance
	 *            the task instance which want to release task group slot.
	 * @throws IllegalArgumentException
	 *             If the taskInstance is null or the task doesn't use task group.
	 */
	...
	private void notifyWaitingTaskInstance(TaskGroupQueue taskGroupQueue) {
		...
		// todo: We may need to add a new status to represent the task instance is
		// waiting for task group slot
		if (taskInstance.getState() != TaskExecutionStatus.SUBMITTED_SUCCESS) {
			throw new UnsupportedOperationException("The TaskInstance: " + taskInstance.getId() + " state is "
					+ taskInstance.getState() + ", no need to notify");
		}
		ProcessInstance processInstance = processInstanceDao.queryById(taskInstance.getProcessInstanceId());
		if (processInstance == null) {
			throw new UnsupportedOperationException("The WorkflowInstance: " + taskInstance.getProcessInstanceId()
					+ " is not exist, no need to notify");
		}
		if (processInstance.getState() != WorkflowExecutionStatus.RUNNING_EXECUTION) {
			throw new UnsupportedOperationException("The WorkflowInstance: " + processInstance.getId() + " state is "
					+ processInstance.getState() + ", no need to notify");
		}
		...
		TaskInstanceWakeupRequest taskInstanceWakeupRequest = TaskInstanceWakeupRequest.builder()
				.processInstanceId(processInstance.getId()).taskInstanceId(taskInstance.getId()).build();
		...
		TaskInstanceWakeupResponse taskInstanceWakeupResponse = iWorkflowInstanceService
				.wakeupTaskInstance(taskInstanceWakeupRequest);
