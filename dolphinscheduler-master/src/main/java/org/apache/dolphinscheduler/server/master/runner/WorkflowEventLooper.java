...
public class WorkflowEventLooper extends BaseDaemonThread implements AutoCloseable {
	...
	@PostConstruct
	public void init() {
		workflowEventHandlerList.forEach(workflowEventHandler -> workflowEventHandlerMap
				.put(workflowEventHandler.getHandleWorkflowEventType(), workflowEventHandler));
	}
	...
	public void run() {
		...
		while (RUNNING_FLAG.get()) {
			...
			try {
				...
				WorkflowEventHandler workflowEventHandler = workflowEventHandlerMap
						.get(workflowEvent.getWorkflowEventType());
				...
				log.error("Handle workflow event error, will drop this event: {}", workflowEvent,
