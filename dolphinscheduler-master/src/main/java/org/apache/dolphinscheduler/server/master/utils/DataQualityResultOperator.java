...
public class DataQualityResultOperator {
	...
	/**
	 * When the task type is data quality, it will get the statistics
	 * value、comparison value、 threshold、check type、operator and failure
	 * strategy，use the formula that {check type} {operator} {threshold} to get dqc
	 * result . If result is failure, it will alert or block
	 * 
	...
	public void operateDqExecuteResult(TaskEvent taskResponseEvent, TaskInstance taskInstance) {
		if (TASK_TYPE_DATA_QUALITY.equals(taskInstance.getTaskType())) {
			...
			ProcessInstance processInstance = processService
					.findProcessInstanceDetailById(taskInstance.getProcessInstanceId()).orElse(null);

			// when the task is failure or cancel, will delete the execute result and
			// statistics value
			...
			DqExecuteResult dqExecuteResult = processService.getDqExecuteResultByTaskInstanceId(taskInstance.getId());
	...
	/**
	 * get the data quality check result and if the result is failure that will
	 * alert or block
	 * 
	 * @param taskResponseEvent
	 ...
	 */
	private void checkDqExecuteResult(TaskEvent taskResponseEvent, DqExecuteResult dqExecuteResult,
