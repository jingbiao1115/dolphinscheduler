...
public class WorkflowExecuteRunnableFactory {
	...
	public Optional<WorkflowExecuteRunnable> createWorkflowExecuteRunnable(Command command)
		...
		try {
			...
			return workflowExecuteRunnableContextOptional
					.map(iWorkflowExecuteContext -> new WorkflowExecuteRunnable(iWorkflowExecuteContext, commandService,
							processService, processInstanceDao, processAlertManager, masterConfig,
							stateWheelExecuteThread, curingGlobalParamsService, taskInstanceDao,
							defaultTaskExecuteRunnableFactory, listenerEventAlertManager, taskGroupCoordinator));
		} catch (Exception ex) {
