...
public class AsyncMasterTaskExecutor extends MasterTaskExecutor {
	...
	protected void executeTask() throws MasterTaskExecuteException {
		...
		// we execute the handle method here, but for async task, this method will not
		// block
		// submit the task to async task queue
		final AsyncTaskExecutionContext asyncTaskExecutionContext = new AsyncTaskExecutionContext(taskExecutionContext,
				iAsyncLogicTask.getAsyncTaskExecuteFunction(), new AsyncTaskCallbackFunctionImpl(this));
