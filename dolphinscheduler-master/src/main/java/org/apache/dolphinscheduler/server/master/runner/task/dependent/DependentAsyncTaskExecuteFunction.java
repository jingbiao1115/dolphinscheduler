...
public class DependentAsyncTaskExecuteFunction implements AsyncTaskExecuteFunction {
	...
	public DependentAsyncTaskExecuteFunction(TaskExecutionContext taskExecutionContext,
			DependentParameters dependentParameters, ProjectDao projectDao, ProcessDefinitionDao processDefinitionDao,
			TaskDefinitionDao taskDefinitionDao, TaskInstanceDao taskInstanceDao,
	...
	private List<DependentExecute> initializeDependentTaskList() {
		...
		final Map<Long, ProcessDefinition> processDefinitionMap = processDefinitionDao
				.queryByCodes(processDefinitionCodes).stream()
		...
		final TaskInstance taskInstance = taskInstanceDao.queryById(taskExecutionContext.getTaskInstanceId());
		List<DependentExecute> dependentExecutes = dependentParameters.getDependTaskList().stream()
				.map(dependentTaskModel -> {
					for (DependentItem dependentItem : dependentTaskModel.getDependItemList()) {
						...
						ProcessDefinition processDefinition = processDefinitionMap
	...
	private DependResult calculateDependResult() {
		...
		return DependentUtils.getDependResultForRelation(this.dependentParameters.getRelation(), dependResultList);
	}
		...
		for (DependentExecute dependentExecute : dependentTaskList) {
			...
			dependentExecute.getDependResultMap().forEach((dependentKey, dependResult) -> {
				if (!dependResultMap.containsKey(dependentKey)) {
					...
					// The log is applied in: api-server obtains the result of the item dependent in
					// the dependent task
					...
					log.info("Dependent item check finished, {} dependentKey: {}, result: {}, dependentDate: {}",
							DEPENDENT_SPLIT, dependentKey, dependResult, dependentDate);
				}
			});
		}
