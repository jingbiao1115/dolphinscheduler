...
/**
 * Master scheduler thread, this thread will consume the commands from database
 * and trigger processInstance executed.
 */
@Service
@Slf4j
public class MasterSchedulerBootstrap extends BaseDaemonThread implements AutoCloseable {
	...
	public void close() throws Exception {
		...
		try (final WorkflowEventLooper workflowEventLooper1 = workflowEventLooper;
	...
	public void run() {
		...
		while (!ServerLifeCycleManager.isStopped()) {
			try {
				...
				// todo: if the workflow event queue is much, we need to handle the back
				// pressure
				...
				commands.parallelStream().forEach(command -> {
					try {
						Optional<WorkflowExecuteRunnable> workflowExecuteRunnableOptional = workflowExecuteRunnableFactory
						...
						ProcessInstance processInstance = workflowExecuteRunnable.getWorkflowExecuteContext()
								.getWorkflowInstance();
						...
						workflowEventQueue
								.addEvent(new WorkflowEvent(WorkflowEventType.START_WORKFLOW, processInstance.getId()));
	...
	private List<Command> findCommands() throws MasterException {
		try {
			...
			final List<Command> result = commandService.findCommandPageBySlot(pageSize, masterCount, thisMasterSlot);
