...
public class TaskExecutionContextFactory {
	...
	public TaskExecutionContext createTaskExecutionContext(TaskInstance taskInstance)
		...
		ResourceParametersHelper resources = Optional
				...
				.map(taskChannel -> taskChannel.getResources(taskInstance.getTaskParams())).orElse(null);
		...
		TaskExecutionContext taskExecutionContext = TaskExecutionContextBuilder.get()
				.buildWorkflowInstanceHost(masterConfig.getMasterAddress()).buildTaskInstanceRelatedInfo(taskInstance)
				.buildTaskDefinitionRelatedInfo(taskInstance.getTaskDefine())
				.buildProcessInstanceRelatedInfo(taskInstance.getProcessInstance())
				.buildProcessDefinitionRelatedInfo(taskInstance.getProcessDefine())
				.buildResourceParametersInfo(resources).buildBusinessParamsMap(businessParamsMap)
				.buildParamInfo(propertyMap).create();
	...
	private void setDataQualityTaskRelation(DataQualityTaskExecutionContext dataQualityTaskExecutionContext,
		...
		dataQualityTaskExecutionContext
				.setHdfsPath(PropertyUtils.getString(Constants.FS_DEFAULT_FS) + PropertyUtils.getString(
						Constants.DATA_QUALITY_ERROR_OUTPUT_PATH, "/user/" + tenantCode + "/data_quality_error_data"));
	...
	private void setSourceConfig(DataQualityTaskExecutionContext dataQualityTaskExecutionContext,
		...
		if (StringUtils.isNotEmpty(config.get(SRC_DATASOURCE_ID))) {
			...
			if (dataSource != null) {
				ConnectorType srcConnectorType = ConnectorType
						.of(DbType.of(Integer.parseInt(config.get(SRC_CONNECTOR_TYPE))).isHive() ? 1 : 0);
	...
	/**
	 * It is used to get comparison params, the param contains comparison
	 * name、comparison table and execute sql. When the type is fixed_value, params
	 * will be null.
	 *
	 * @param dataQualityTaskExecutionContext
	 * @param config
	...
	private void setComparisonParams(DataQualityTaskExecutionContext dataQualityTaskExecutionContext,
			Map<String, String> config, List<DqRuleInputEntry> ruleInputEntryList,
	...
	/**
	 * The TargetConfig will be used in DataQualityApplication that get the data
	 * which be used to compare to src value
	 *
	 * @param dataQualityTaskExecutionContext
	 * @param config
	 */
	private void setTargetConfig(DataQualityTaskExecutionContext dataQualityTaskExecutionContext,
		...
		if (StringUtils.isNotEmpty(config.get(TARGET_DATASOURCE_ID))) {
			...
			if (dataSource != null) {
				ConnectorType targetConnectorType = ConnectorType
						.of(DbType.of(Integer.parseInt(config.get(TARGET_CONNECTOR_TYPE))).isHive() ? 1 : 0);
	 ...
	 * quality check result into dolphin scheduler datasource
	 *
	 * @param dataQualityTaskExecutionContext
	 */
