...
public class StateEventResponseService {
	...
	private void persist(StateEvent stateEvent) {
		try {
			...
			WorkflowExecuteRunnable workflowExecuteThread = this.processInstanceExecCacheManager
					.getByProcessInstanceId(stateEvent.getProcessInstanceId());
			// We will refresh the task instance status first, if the refresh failed the
			// event will not be removed
