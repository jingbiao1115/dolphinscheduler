...
public class SwitchLogicTask extends BaseSyncLogicTask<SwitchParameters> {
	...
	public void handle() throws MasterTaskExecuteException {
		...
		TaskExecutionStatus status = (conditionResult == DependResult.SUCCESS)
	...
	private DependResult calculateConditionResult() {
		...
		Map<String, Property> globalParams = JSONUtils.toList(processInstance.getGlobalParams(), Property.class)
				.stream().collect(Collectors.toMap(Property::getProp, Property -> Property));
		Map<String, Property> varParams = JSONUtils.toList(taskInstance.getVarPool(), Property.class).stream()
		...
		for (SwitchResultVo info : switchResultVos) {
			...
			String content = SwitchTaskUtils.generateContentWithTaskParams(info.getCondition(), globalParams,
					varParams);
