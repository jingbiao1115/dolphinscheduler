...
public class TaskDelayEventHandler implements TaskEventHandler {
	...
	public void handleTaskEvent(TaskEvent taskEvent) throws TaskEventHandleError {
		...
		WorkflowExecuteRunnable workflowExecuteRunnable = this.processInstanceExecCacheManager
		...
		if (taskInstance.getState().isFinished()) {
			log.warn(
					...
					taskInstance.getState(), taskEvent);
			sendAckToWorker(taskEvent);
			return;
		}
		...
		TaskStateEvent stateEvent = TaskStateEvent.builder().processInstanceId(taskEvent.getProcessInstanceId())
				.taskInstanceId(taskEvent.getTaskInstanceId()).status(taskEvent.getState())
