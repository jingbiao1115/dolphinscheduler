...
public class MasterFailoverService {
	...
	public void checkMasterFailover() {
		List<String> needFailoverMasterHosts = processService.queryNeedFailoverProcessInstanceHost().stream()
	 ...
	 * <p>
	 * When the process instance belongs to the given masterHost and the restartTime
	 * is before the current server start up time, then the process instance will be
	 * failovered.
	 *
	 * @param masterHost
	 *            master host
	...
	private void doFailoverMaster(@NonNull String masterHost) {
		...
		Optional<Date> masterStartupTimeOptional = getServerStartupTime(
				registryClient.getServerList(RegistryNodeType.MASTER), masterHost);
		List<ProcessInstance> needFailoverProcessInstanceList = processService
				.queryNeedFailoverProcessInstances(masterHost);
		...
		log.info("Master[{}] failover finished, useTime:{}ms", masterHost,
	...
	private boolean checkProcessInstanceNeedFailover(Optional<Date> beFailoveredMasterStartupTimeOptional,
		...
		// The process has already been failover, since when we do master failover we
		// will hold a lock, so we can
		// guarantee
