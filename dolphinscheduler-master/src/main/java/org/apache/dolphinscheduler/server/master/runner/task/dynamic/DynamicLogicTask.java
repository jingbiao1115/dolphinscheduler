...
public class DynamicLogicTask extends BaseAsyncLogicTask<DynamicParameters> {
	...
	public DynamicLogicTask(TaskExecutionContext taskExecutionContext, ProcessInstanceDao processInstanceDao,
			TaskInstanceDao taskInstanceDao, SubWorkflowService subWorkflowService, ProcessService processService,
			ProcessDefinitionMapper processDefineMapper, CommandMapper commandMapper) {
	...
	public AsyncTaskExecuteFunction getAsyncTaskExecuteFunction() throws MasterTaskExecuteException {
		...
		return new DynamicAsyncTaskExecuteFunction(taskExecutionContext, processInstance, taskInstance, this,
				commandMapper, subWorkflowService, taskParameters.getDegreeOfParallelism());
	}
	...
	public List<Map<String, String>> generateParameterGroup() {
		...
		List<Map<String, String>> parameterGroup = cartesianProduct.stream()
				.map(inputParameterList -> inputParameterList.stream()
						.collect(Collectors.toMap(DynamicInputParameter::getName, DynamicInputParameter::getValue)))
				.collect(Collectors.toList());
	...
	private void changeRunningSubprocessInstancesToStop(WorkflowExecutionStatus stopStatus)
		...
		for (ProcessInstance subProcessInstance : runningSubProcessInstanceList) {
			...
			try {
				...
				throw new MasterTaskExecuteException(String.format(
						"Send stop request to SubWorkflow's master: %s failed", subProcessInstance.getHost()), e);
			}
		}
	}
