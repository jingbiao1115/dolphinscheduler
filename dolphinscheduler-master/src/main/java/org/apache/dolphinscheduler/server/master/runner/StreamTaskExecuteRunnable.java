...
public class StreamTaskExecuteRunnable implements Runnable {
	...
	public StreamTaskExecuteRunnable(TaskDefinition taskDefinition,
		...
		this.streamTaskInstanceExecCacheManager = SpringApplicationContext
				.getBean(StreamTaskInstanceExecCacheManager.class);
	...
	public void run() {
		...
		List<ProcessTaskRelation> processTaskRelationList = processTaskRelationMapper
				.queryByTaskCode(taskDefinition.getCode());
	...
	public void handleEvents() {
		...
		while (!this.taskEvents.isEmpty()) {
			try {
				...
				log.error("State event handle error, will retry this event: {}", taskEvent, stateEventHandleException);
				ThreadUtils.sleep(Constants.SLEEP_TIME_MILLIS);
			} catch (Exception e) {
				// we catch the exception here, since if the state event handle failed, the
				// state event will still keep
	...
	protected TaskExecutionContext getTaskExecutionContext(TaskInstance taskInstance) {
		...
		TaskExecutionContext taskExecutionContext = TaskExecutionContextBuilder.get()
				...
				.buildTaskDefinitionRelatedInfo(taskDefinition).buildResourceParametersInfo(resources)
				.buildBusinessParamsMap(new HashMap<>()).buildParamInfo(propertyMap).create();
	...
	private void sendAckToWorker(TaskEvent taskEvent) {
		// If event handle success, send ack to worker to otherwise the worker will
		// retry this event
		ITaskInstanceExecutionEventAckListener instanceExecutionEventAckListener = SingletonJdkDynamicRpcClientProxyFactory
