...
public class ProcessInstanceMetrics {
	...
	private final Set<String> processInstanceStates = ImmutableSet.of("submit", "timeout", "finish", "failover",
			"success", "fail", "stop");

	static {
		for (final String state : processInstanceStates) {
			Counter.builder("ds.workflow.instance.count").tags("state", state, "process.definition.code", "dummy")
	...
	private final Timer commandQueryTimer = Timer.builder("ds.workflow.command.query.duration")
			.description("Command query duration").register(Metrics.globalRegistry);

	private final Timer processInstanceGenerateTimer = Timer.builder("ds.workflow.instance.generate.duration")
			.description("Process instance generated duration").register(Metrics.globalRegistry);
	...
	public void incProcessInstanceByStateAndProcessDefinitionCode(final String state,
		...
		// you have no choice but to construct or lookup the Meter inside your method
		// body.
		// The lookup cost is just a single hash lookup, so it is acceptable for most
		// use cases.
	...
	public void cleanUpProcessInstanceCountMetricsByDefinitionCode(final Long processDefinitionCode) {
		for (final String state : processInstanceStates) {
			final Counter counter = Metrics.globalRegistry.counter("ds.workflow.instance.count", "state", state,
