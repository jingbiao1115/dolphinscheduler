	...
	public TaskInstanceWakeupResponse operate(TaskInstanceWakeupRequest taskInstanceWakeupRequest) {
		try {
			...
			WorkflowExecuteRunnable workflowExecuteRunnable = processInstanceExecCacheManager
					.getByProcessInstanceId(workflowInstanceId);
			...
			DefaultTaskExecuteRunnable defaultTaskExecuteRunnable = workflowExecuteRunnable
					.getTaskExecuteRunnableById(taskInstanceId).orElse(null);
