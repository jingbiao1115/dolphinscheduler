...
public class DependentExecute {
	 ...
	 */
	private final TaskDefinitionLogDao taskDefinitionLogDao = SpringApplicationContext
			.getBean(TaskDefinitionLogDao.class);
	...
	private DependResult getDependentResultForItem(DependentItem dependentItem, Date currentTime, int testFlag) {
		List<DateInterval> dateIntervals = DependentUtils.getDateIntervalList(currentTime,
	...
	private DependResult dependResultByAllTaskOfProcessInstance(ProcessInstance processInstance, int testFlag) {
		...
		if (processInstance.getState().isSuccess()) {
			...
			Map<Long, String> taskDefinitionCodeMap = taskDefinitionLogs.stream()
					.filter(taskDefinitionLog -> taskDefinitionLog.getFlag() == Flag.YES)
					.collect(Collectors.toMap(TaskDefinitionLog::getCode, TaskDefinitionLog::getName));

			List<TaskInstance> taskInstanceList = taskInstanceDao.queryLastTaskInstanceListIntervalInProcessInstance(
					processInstance.getId(), taskDefinitionCodeMap.keySet(), testFlag);
			Map<Long, TaskExecutionStatus> taskExecutionStatusMap = taskInstanceList.stream()
	 ...
	 *            last process instance in the date interval
	 * @param depTaskCode
	 *            the dependent task code
	 * @param testFlag
	 *            test flag
	...
	private DependResult dependResultBySingleTaskInstance(ProcessInstance processInstance, long depTaskCode,
			int testFlag) {
		TaskInstance taskInstance = taskInstanceDao
				.queryLastTaskInstanceIntervalInProcessInstance(processInstance.getId(), depTaskCode, testFlag);
	...
	/**
	 * find the last one process instance that : 1. manual run and finish between
	 * the interval 2. schedule run and schedule time between the interval
	 *
	...
	private ProcessInstance findLastProcessInterval(Long definitionCode, DateInterval dateInterval, int testFlag) {
		...
		ProcessInstance lastSchedulerProcess = processInstanceDao.queryLastSchedulerProcessInterval(definitionCode,
				dateInterval, testFlag);

		ProcessInstance lastManualProcess = processInstanceDao.queryLastManualProcessInterval(definitionCode,
				dateInterval, testFlag);
		...
		// In the time range, there are both manual and scheduled workflow instances,
		// return the last workflow instance
	...
	/**
	 * check for first-running query the first processInstance by scheduleTime(or
	 * startTime if scheduleTime is null)
	 * 
	 * @param dependentItem
	 * @return
	 */
	public boolean isFirstProcessInstance(DependentItem dependentItem) {
		ProcessInstance firstProcessInstance = processInstanceDao
				.queryFirstScheduleProcessInstance(dependentItem.getDefinitionCode());
