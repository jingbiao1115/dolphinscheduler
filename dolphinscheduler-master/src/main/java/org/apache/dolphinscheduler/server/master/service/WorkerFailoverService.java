...
public class WorkerFailoverService {
	...
	public WorkerFailoverService(@NonNull RegistryClient registryClient, @NonNull MasterConfig masterConfig,
			@NonNull ProcessService processService, @NonNull WorkflowExecuteThreadPool workflowExecuteThreadPool,
			@NonNull ProcessInstanceExecCacheManager cacheManager, @NonNull TaskInstanceDao taskInstanceDao) {
	 ...
	 * tasks belong the given worker, and failover these tasks.
	 * <p>
	...
	public void failoverWorker(@NonNull String workerHost) {
		...
		// we query the task instance from cache, so that we can directly update the
		// cache
		final Optional<Date> needFailoverWorkerStartTime = getServerStartupTime(
				registryClient.getServerList(RegistryNodeType.WORKER), workerHost);
		...
		for (TaskInstance taskInstance : needFailoverTaskInstanceList) {
			try {
				...
				ProcessInstance processInstance = processInstanceCacheMap
						.computeIfAbsent(taskInstance.getProcessInstanceId(), k -> {
							WorkflowExecuteRunnable workflowExecuteRunnable = cacheManager
									.getByProcessInstanceId(taskInstance.getProcessInstanceId());
		...
		log.info("Worker[{}] failover finished, useTime:{}ms", workerHost,
	 ...
	 * 2. change task state from running to need failover. 3. try to notify local
	 * master
	 *
	...
	private void failoverTaskInstance(@NonNull ProcessInstance processInstance, @NonNull TaskInstance taskInstance) {
		...
		TaskStateEvent stateEvent = TaskStateEvent.builder().processInstanceId(processInstance.getId())
				.taskInstanceId(taskInstance.getId()).status(TaskExecutionStatus.NEED_FAULT_TOLERANCE)
	...
	private boolean checkTaskInstanceNeedFailover(Optional<Date> needFailoverWorkerStartTime,
			@Nullable ProcessInstance processInstance, TaskInstance taskInstance) {
		...
		if (!StringUtils.equalsIgnoreCase(processInstance.getHost(), localAddress)) {
			log.error(
					...
					processInstance.getHost(), localAddress);
			return false;
		}
		...
		if (taskInstance.getSubmitTime() != null
				&& taskInstance.getSubmitTime().after(needFailoverWorkerStartTime.get())) {
			log.info(
					...
					taskInstance.getSubmitTime(), needFailoverWorkerStartTime.get());
			return false;
		}
	...
	private List<TaskInstance> getNeedFailoverTaskInstance(@NonNull String failoverWorkerHost) {
		// we query the task instance from cache, so that we can directly update the
		// cache
		return cacheManager.getAll().stream()
	...
	private void killYarnTask(TaskInstance taskInstance, ProcessInstance processInstance) {
		try {
			...
			TaskExecutionContext taskExecutionContext = TaskExecutionContextBuilder.get()
					...
					.buildTaskInstanceRelatedInfo(taskInstance).buildProcessInstanceRelatedInfo(processInstance)
					.buildProcessDefinitionRelatedInfo(processInstance.getProcessDefinition()).create();
			...
			ILogService iLogService = SingletonJdkDynamicRpcClientProxyFactory.getProxyClient(taskInstance.getHost(),
					ILogService.class);
			GetAppIdResponse getAppIdResponse = iLogService
					.getAppId(new GetAppIdRequest(taskInstance.getId(), taskInstance.getLogPath()));
