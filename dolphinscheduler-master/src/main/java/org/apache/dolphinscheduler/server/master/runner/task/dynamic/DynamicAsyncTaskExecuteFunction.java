...
public class DynamicAsyncTaskExecuteFunction implements AsyncTaskExecuteFunction {
	...
	public DynamicAsyncTaskExecuteFunction(TaskExecutionContext taskExecutionContext, ProcessInstance processInstance,
			TaskInstance taskInstance, DynamicLogicTask dynamicLogicTask, CommandMapper commandMapper,
			SubWorkflowService subWorkflowService, int degreeOfParallelism) {
	...
	public @NonNull AsyncTaskExecutionStatus getAsyncTaskExecutionStatus() {
		...
		List<ProcessInstance> finishedSubProcessInstance = subWorkflowService
				.filterFinishProcessInstances(allSubProcessInstance);
	...
	private void setOutputParameters() {
		...
		for (ProcessInstance processInstance : allSubProcessInstance) {
			...
			Map<String, String> dynamicParams = JSONUtils
					.toMap(JSONUtils.toMap(processInstance.getCommandParam()).get(CMD_DYNAMIC_START_PARAMS));
	...
	private void startSubProcessInstances(List<ProcessInstance> allSubProcessInstance, int startCount) {
		List<ProcessInstance> waitingProcessInstances = subWorkflowService
				.filterWaitToRunProcessInstances(allSubProcessInstance);
