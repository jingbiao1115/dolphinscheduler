...
public class WorkflowExecuteRunnableTest {
	...
	public void init() throws Exception {
		...
		workflowExecuteThread = Mockito.spy(new WorkflowExecuteRunnable(workflowExecuteContext, commandService,
				processService, processInstanceDao, processAlertManager, config, stateWheelExecuteThread,
				curingGlobalParamsService, taskInstanceDao, defaultTaskExecuteRunnableFactory,
				listenerEventAlertManager, taskGroupCoordinator));
	}
	...
	public void testParseStartNodeName() {
		try {
			...
			List<String> nodeNames = (List<String>) method.invoke(workflowExecuteThread,
	...
	public void testGetStartTaskInstanceList() {
		try {
			...
			Mockito.when(taskInstanceDao.queryByIds(Arrays.asList(taskInstance1.getId(), taskInstance2.getId(),
					taskInstance3.getId(), taskInstance4.getId())))
			...
			method.setAccessible(true);
			List<TaskInstance> taskInstances = workflowExecuteThread
					.getRecoverTaskInstanceList(JSONUtils.toJsonString(cmdParam));
			Assertions.assertEquals(4, taskInstances.size());

			cmdParam.put(CMD_PARAM_RECOVERY_START_NODE_STRING, "1");
			List<TaskInstance> taskInstanceEmpty = (List<TaskInstance>) method.invoke(workflowExecuteThread,
					JSONUtils.toJsonString(cmdParam));
	...
	public void testClearDataIfExecuteTask() throws NoSuchFieldException, IllegalAccessException {
		...
		Mockito.when(taskInstanceDao.queryByWorkflowInstanceIdAndTaskCode(processInstance.getId(),
				taskInstance1.getTaskCode())).thenReturn(taskInstance1);
		Mockito.when(taskInstanceDao.queryByWorkflowInstanceIdAndTaskCode(processInstance.getId(),
				taskInstance2.getTaskCode())).thenReturn(null);
