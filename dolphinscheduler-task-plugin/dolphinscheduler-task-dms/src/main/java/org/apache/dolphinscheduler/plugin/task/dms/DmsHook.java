...
public class DmsHook {
	...
	public static AWSDatabaseMigrationService createClient() {
		...
		// create a DMS client
		return AWSDatabaseMigrationServiceClientBuilder.standard().withCredentials(awsCredentialsProvider)
	...
	public Boolean createReplicationTask() throws Exception {
		...
		CreateReplicationTaskRequest request = new CreateReplicationTaskRequest()
				...
				.withTargetEndpointArn(targetEndpointArn).withReplicationInstanceArn(replicationInstanceArn)
				.withMigrationType(migrationType).withTableMappings(tableMappings)
				.withReplicationTaskSettings(replicationTaskSettings).withCdcStartTime(cdcStartTime)
				.withCdcStartPosition(cdcStartPosition).withCdcStopPosition(cdcStopPosition).withTags(tags)
				.withTaskData(taskData).withResourceIdentifier(resourceIdentifier);
	...
	public Boolean startReplicationTask() {
		...
		StartReplicationTaskRequest request = new StartReplicationTaskRequest()
				.withReplicationTaskArn(replicationTaskArn).withStartReplicationTaskType(startReplicationTaskType)
				.withCdcStartTime(cdcStartTime).withCdcStartPosition(cdcStartPosition)
	...
	public Boolean awaitConnectSuccess(String replicationInstanceArn, String endpointArn) {
		Filter instanceFilters = new Filter().withName(AWS_KEY.REPLICATION_INSTANCE_ARN)
				.withValues(replicationInstanceArn);
		...
		DescribeConnectionsRequest request = new DescribeConnectionsRequest()
				.withFilters(endpointFilters, instanceFilters).withMarker("");
		while (true) {
	...
	public ReplicationTask describeReplicationTasks() {
		Filter replicationTaskFilter = new Filter().withName(AWS_KEY.REPLICATION_TASK_ARN)
				.withValues(replicationTaskArn);
