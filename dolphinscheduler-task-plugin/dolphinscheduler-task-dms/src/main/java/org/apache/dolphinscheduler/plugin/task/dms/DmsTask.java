...
public class DmsTask extends AbstractRemoteTask {
	...
	private static final ObjectMapper objectMapper = JsonMapper.builder().configure(FAIL_ON_UNKNOWN_PROPERTIES, false)
			.configure(ACCEPT_EMPTY_ARRAY_AS_NULL_OBJECT, true).configure(READ_UNKNOWN_ENUM_VALUES_AS_NULL, true)
			.configure(REQUIRE_SETTERS_FOR_GETTERS, true)
			.propertyNamingStrategy(new PropertyNamingStrategy.UpperCamelCaseStrategy()).build();
	/**
	...
	public void trackApplicationStatus() {
		...
		// if CdcStopPosition is not set, the task will not continue to check the
		// running status
	...
	public int checkCreateReplicationTask() throws TaskException {
		...
		// if create replication task successfully, return EXIT_CODE_SUCCESS, else
	...
	public int startReplicationTask() {
		...
		try {
			...
			// Only restart task when the error contains "Test connection", means instance
			// can not connect to source or
			// target
			...
			// if only restart task, run dmsHook.describeReplicationTasks to get replication
			// task arn
			if (parameters.getIsRestartTask()) {
	 ...
	 *
	 * @return true if stop task when cdc type and cdcStopPosition is not set, else
	 *         return false
	 */
	...
	public void convertJsonParameters() throws TaskException {
		// create a new parameter object using the json data if the json data is not
		// empty
