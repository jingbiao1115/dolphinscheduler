...
public class EmrAddStepsTask extends AbstractEmrTask {
	...
	private final HashSet<String> waitingStateSet = Sets.newHashSet(StepState.PENDING.toString(),
			StepState.CANCEL_PENDING.toString(), StepState.RUNNING.toString());
	...
	protected AddJobFlowStepsRequest createAddJobFlowStepsRequest() {
		...
		try {
			addJobFlowStepsRequest = objectMapper.readValue(emrParameters.getStepsDefineJson(),
					AddJobFlowStepsRequest.class);
		} catch (JsonProcessingException e) {
			throw new EmrTaskException("can not parse AddJobFlowStepsRequest from json", e);
		}

		// When a single task definition is associated with multiple steps, the state
		// tracking will have high
		// complexity.
		// Therefore, A task definition only supports the association of a single step,
		// which can better ensure the
	 ...
	 *
	 * @param stepStatus
	 *            aws emr execution status details of the cluster step.
	...
	public void cancelApplication() throws TaskException {
		...
		CancelStepsInfo cancelEmrStepInfo = cancelStepsResult.getCancelStepsInfoList().stream()
				.filter(cancelStepsInfo -> cancelStepsInfo.getStepId().equals(stepId)).findFirst()
