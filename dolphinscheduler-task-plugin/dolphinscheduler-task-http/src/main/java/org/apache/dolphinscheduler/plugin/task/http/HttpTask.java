...
public class HttpTask extends AbstractTask {
	...
	public void handle(TaskCallBack taskCallBack) throws TaskException {
		...
		try (CloseableHttpClient client = createHttpClient(); CloseableHttpResponse response = sendRequest(client)) {
			...
			log.info(
					...
					formatTimeStamp, httpParameters.getUrl(), httpParameters.getHttpMethod(), costTime, statusCode,
					body, output);
		} catch (Exception e) {
		...
		switch (httpParameters.getHttpCheckCondition()) {
			case BODY_CONTAINS :
				if (StringUtils.isEmpty(body) || !body.contains(httpParameters.getCondition())) {
					appendMessage(httpParameters.getUrl() + " doesn contain " + httpParameters.getCondition());
					exitStatusCode = -1;
				}
				break;
	...
	protected void addRequestParams(RequestBuilder builder, List<HttpProperty> httpPropertyList, String httpBody) {
		if (StringUtils.isNotEmpty(httpBody)) {
			builder.setEntity(new StringEntity(httpBody,
		...
		if (CollectionUtils.isNotEmpty(httpPropertyList)) {
			...
			if (builder.getEntity() == null) {
				builder.setEntity(new StringEntity(jsonParam.toString(),
						ContentType.create(ContentType.APPLICATION_JSON.getMimeType(), StandardCharsets.UTF_8)));
			}
		}
	}
	...
	protected CloseableHttpClient createHttpClient()
		...
		Registry<ConnectionSocketFactory> registry = RegistryBuilder.<ConnectionSocketFactory>create()
				.register("http", PlainConnectionSocketFactory.INSTANCE).register("https", createIgnoreVerifySSL())
				.build();
		// 创建一个ConnectionManager
		PoolingHttpClientConnectionManager pool = new PoolingHttpClientConnectionManager(registry);
		// 定制CloseableHttpClient对象
		// HttpClientBuilder httpClientBuilder =
		// HttpClients.custom().setConnectionManager(pool).setSSLSocketFactory(sslsf);
		...
		HttpClientBuilder httpClientBuilder = HttpClients.custom().setDefaultRequestConfig(requestConfig)
				.setConnectionManager(pool).setSSLSocketFactory(sslsf);
