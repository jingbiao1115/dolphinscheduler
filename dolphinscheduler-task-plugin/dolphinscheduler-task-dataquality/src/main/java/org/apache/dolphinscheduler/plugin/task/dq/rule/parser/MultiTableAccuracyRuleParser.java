...
public class MultiTableAccuracyRuleParser implements IRuleParser {
	...
	public DataQualityConfiguration parse(Map<String, String> inputParameterValue,
		...
		List<DqRuleExecuteSql> dqRuleExecuteSqlList = JSONUtils.toList(context.getExecuteSqlList(),
				DqRuleExecuteSql.class);

		DqRuleExecuteSql statisticsSql = RuleParserUtils
				.getExecuteSqlListByType(dqRuleExecuteSqlList, ExecuteSqlType.STATISTICS).get(0);
		...
		List<MappingColumn> mappingColumnList = RuleParserUtils
				.getMappingColumnList(inputParameterValue.get(MAPPING_COLUMNS));
		...
		index = RuleParserUtils.replaceExecuteSqlPlaceholder(dqRuleExecuteSqlList, index, inputParameterValue,
		...
		List<BaseConfig> writerConfigList = RuleParserUtils.getAllWriterConfigList(inputParameterValue, context, index,
				transformerConfigList, writerSql, RuleManager.TASK_STATISTICS_VALUE_WRITER_SQL);

		return new DataQualityConfiguration(context.getRuleName(), RuleParserUtils.getEnvConfig(), readerConfigList,
				writerConfigList, transformerConfigList);
	}
}
