 ...
 * DataQualityConfiguration, which will be converted into a string as the
 * parameter of DataQualityApplication, and DataQualityApplication is spark
 * application
 */
@Slf4j
public class DataQualityTask extends AbstractYarnTask {
	...
	public void init() {
		...
		dataQualityParameters = JSONUtils.parseObject(dqTaskExecutionContext.getTaskParams(),
		...
		DataQualityTaskExecutionContext dataQualityTaskExecutionContext = dqTaskExecutionContext
	...
	private void operateInputParameter(Map<String, String> inputParameter,
		...
		if (StringUtils.isNotEmpty(dataQualityTaskExecutionContext.getHdfsPath())) {
			inputParameter.put(ERROR_OUTPUT_PATH,
					dataQualityTaskExecutionContext.getHdfsPath() + SLASH + dqTaskExecutionContext.getProcessDefineId()
							+ UNDERLINE + dqTaskExecutionContext.getProcessInstanceId() + UNDERLINE
	...
	private static String replaceDoubleBrackets(String mainParameter) {
		mainParameter = mainParameter.replace(Constants.DOUBLE_BRACKETS_LEFT, Constants.DOUBLE_BRACKETS_LEFT_SPACE)
