...
/**
 * RuleManager is responsible for parsing the input parameters to the
 * DataQualityConfiguration And DataQualityConfiguration will be used in
...
public class RuleManager {
	...
	private static final String BASE_SQL = "select ${rule_type} as rule_type," + "${rule_name} as rule_name,"
			+ "${process_definition_id} as process_definition_id," + "${process_instance_id} as process_instance_id,"
			+ "${task_instance_id} as task_instance_id," + "${statistics_name} AS statistics_value,"
			+ "${comparison_name} AS comparison_value," + "${comparison_type} AS comparison_type,"
			+ "${check_type} as check_type," + "${threshold} as threshold," + "${operator} as operator,"
			+ "${failure_strategy} as failure_strategy," + "'${error_output_path}' as error_output_path,"
			+ "${create_time} as create_time," + "${update_time} as update_time ";

	public static final String DEFAULT_COMPARISON_WRITER_SQL = BASE_SQL
			+ "from ${statistics_table} full join ${comparison_table}";

	public static final String MULTI_TABLE_COMPARISON_WRITER_SQL = BASE_SQL + "from ( ${statistics_execute_sql} ) tmp1 "
			+ "join ( ${comparison_execute_sql} ) tmp2";

	public static final String SINGLE_TABLE_CUSTOM_SQL_WRITER_SQL = BASE_SQL + "from ( ${statistics_table} ) tmp1 "
			+ "join ${comparison_table}";
	public static final String TASK_STATISTICS_VALUE_WRITER_SQL = "select "
			+ "${process_definition_id} as process_definition_id," + "${task_instance_id} as task_instance_id,"
			+ "${rule_id} as rule_id," + "${unique_code} as unique_code," + "'${statistics_name}'AS statistics_name,"
			+ "${statistics_name} AS statistics_value," + "${data_time} as data_time,"
			+ "${create_time} as create_time," + "${update_time} as update_time " + "from ${statistics_table}";
	...
	public DataQualityConfiguration generateDataQualityParameter() throws RuntimeException {
		...
		Map<String, String> inputParameterValueResult = RuleParserUtils.getInputParameterMapFromEntryList(
