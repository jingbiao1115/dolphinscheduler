...
public abstract class AbstractCommandExecutor {
	...
	public AbstractCommandExecutor(Consumer<LinkedBlockingQueue<String>> logHandler, TaskExecutionContext taskRequest) {
		...
		if (this.taskRequest != null) {
			// set logBufferEnable=true if the task uses logHandler and logBuffer to buffer
			// log messages
			this.taskRequest.setLogBufferEnable(true);
		}
	}

	// todo: We need to build the IShellActuator in outer class, since different
	// task may have specific logic to build
	// the IShellActuator
	public TaskResponse run(IShellInterceptorBuilder iShellInterceptorBuilder, TaskCallBack taskCallBack)
			throws Exception {
		TaskResponse result = new TaskResponse();
		int taskInstanceId = taskRequest.getTaskInstanceId();
		// todo: we need to use state like JDK Thread to make sure the killed task
		// should not be executed
		iShellInterceptorBuilder = iShellInterceptorBuilder.shellDirectory(taskRequest.getExecutePath())
		...
		TaskExecutionStatus kubernetesStatus = ProcessUtils
				.getApplicationStatus(taskRequest.getK8sTaskExecutionContext(), taskRequest.getTaskAppId());
