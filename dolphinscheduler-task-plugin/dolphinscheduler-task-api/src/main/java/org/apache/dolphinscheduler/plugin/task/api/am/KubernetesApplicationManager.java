...
public class KubernetesApplicationManager implements ApplicationManager {
	...
	public boolean killApplication(ApplicationManagerContext applicationManagerContext) throws TaskException {
		...
		FilterWatchListDeletable<Pod, PodList, PodResource> watchList = getListenPod(
	 ...
	 * @return
	 */
	private FilterWatchListDeletable<Pod, PodList, PodResource> getListenPod(
	...
	private KubernetesClient getClient(KubernetesApplicationManagerContext kubernetesApplicationManagerContext) {
		K8sTaskExecutionContext k8sTaskExecutionContext = kubernetesApplicationManagerContext
	 ...
	 */
	public TaskExecutionStatus getApplicationStatus(
			KubernetesApplicationManagerContext kubernetesApplicationManagerContext) throws TaskException {
	...
	public LogWatch getPodLogWatcher(KubernetesApplicationManagerContext kubernetesApplicationManagerContext) {
		...
		return client.pods().inNamespace(pod.getMetadata().getNamespace()).withName(pod.getMetadata().getName())
				.inContainer(kubernetesApplicationManagerContext.getContainerName()).watchLog();
	}
