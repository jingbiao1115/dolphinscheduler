...
public class K8sUtils {
	...
	public Boolean jobExist(String jobName, String namespace) {
		...
		try {
			...
			result = jobs.stream().filter(job -> job.getMetadata().getName().equals(jobName)).findFirst();
	...
	public Watch createBatchJobWatcher(String jobName, Watcher<Job> watcher) {
		try {
			return client.batch().v1().jobs().withName(jobName).watch(watcher);
		} catch (Exception e) {
	...
	public String getPodLog(String jobName, String namespace) {
		try {
			...
			return client.pods().inNamespace(namespace).withName(podName).tailingLines(LOG_LINES).getLog(Boolean.TRUE);
		} catch (Exception e) {
