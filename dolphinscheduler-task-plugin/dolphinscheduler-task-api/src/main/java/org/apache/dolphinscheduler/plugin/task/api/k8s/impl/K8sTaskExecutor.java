...
public class K8sTaskExecutor extends AbstractK8sTaskExecutor {
	...
	public void buildK8sJob(K8sTaskMainParameters k8STaskMainParameters) {
		...
		Affinity affinity = k8STaskMainParameters.getNodeSelectorRequirements().size() == 0
				? null
				: new AffinityBuilder().withNewNodeAffinity().withNewRequiredDuringSchedulingIgnoredDuringExecution()
						.addNewNodeSelectorTermLike(nodeSelectorTerm).endNodeSelectorTerm()
						.endRequiredDuringSchedulingIgnoredDuringExecution().endNodeAffinity().build();

		job = new JobBuilder().withApiVersion(API_VERSION).withNewMetadata().withName(k8sJobName)
				.withLabels(jobLabelMap).withNamespace(namespaceName).endMetadata().withNewSpec()
				.withTtlSecondsAfterFinished(JOB_TTL_SECONDS).withNewTemplate().withNewMetadata()
				.withLabels(podLabelMap).endMetadata().withNewSpec().addNewContainer().withName(k8sJobName)
				.withImage(image).withCommand(commands.size() == 0 ? null : commands)
				.withArgs(args.size() == 0 ? null : args).withImagePullPolicy(imagePullPolicy)
				.withResources(new ResourceRequirements(limitRes, reqRes)).withEnv(envVars).endContainer()
				.withImagePullSecrets(
						StringUtils.isEmpty(pullSecret) ? null : singletonList(new LocalObjectReference(pullSecret)))
				.withRestartPolicy(RESTART_POLICY).withAffinity(affinity).endSpec().endTemplate()
				.withBackoffLimit(retryNum).endSpec().build();
	...
	private void parsePodLogOutput() {
		...
		podLogOutputFuture = collectPodLogExecutorService.submit(() -> {
			...
			LogUtils.setWorkflowAndTaskInstanceIDMDC(taskRequest.getProcessInstanceId(),
					taskRequest.getTaskInstanceId());
			LogUtils.setTaskInstanceLogFullPathMDC(taskRequest.getLogPath());
			try (LogWatch watcher = ProcessUtils.getPodLogWatcher(taskRequest.getK8sTaskExecutionContext(),
	...
	public void submitJob2k8s(String k8sParameterStr) {
		int taskInstanceId = taskRequest.getTaskInstanceId();
		String taskName = taskRequest.getTaskName().toLowerCase(Locale.ROOT);
		K8sTaskMainParameters k8STaskMainParameters = JSONUtils.parseObject(k8sParameterStr,
