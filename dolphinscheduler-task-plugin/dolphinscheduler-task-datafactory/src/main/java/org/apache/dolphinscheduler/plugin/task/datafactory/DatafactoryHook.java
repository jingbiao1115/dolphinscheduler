...
public class DatafactoryHook {
	...
	public static DatafactoryStatus[] taskFinishFlags = {DatafactoryStatus.Failed, DatafactoryStatus.Succeeded,
	...
	protected static DataFactoryManager createClient() {
		...
		credential = new ClientSecretCredentialBuilder().clientId(AZURE_CLIENT_ID).clientSecret(AZURE_CLIENT_SECRET)
				.tenantId(AZURE_SECRET_TENANT_ID).authorityHost(profile.getEnvironment().getActiveDirectoryEndpoint())
				.build();
	...
	public DatafactoryStatus queryDatafactoryTaskStatus(DatafactoryParameters parameters) {
		...
		PipelineRuns pipelineRuns = client.pipelineRuns();
		PipelineRun pipelineRun = pipelineRuns.get(parameters.getResourceGroupName(), parameters.getFactoryName(),
				runId);
