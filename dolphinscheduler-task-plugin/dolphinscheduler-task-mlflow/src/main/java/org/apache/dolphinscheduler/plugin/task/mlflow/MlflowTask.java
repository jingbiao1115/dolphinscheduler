...
public class MlflowTask extends AbstractTask {
	...
	public void handle(TaskCallBack taskCallBack) throws TaskException {
		try {
			...
			IShellInterceptorBuilder<?, ?> shellActuatorBuilder = ShellInterceptorBuilderFactory.newBuilder()
					.properties(ParameterUtils.convert(getParamsMap())).appendScript(buildCommand());
	...
	private String buildCommandForMlflowProjects() {
		...
		switch (mlflowParameters.getMlflowJobType()) {
			...
			case MlflowConstants.JOB_TYPE_CUSTOM_PROJECT :
				...
				runCommand = String.format(runCommand, mlflowParameters.getParams(),
						mlflowParameters.getExperimentName());
				break;
	...
	public int checkDockerHealth() {
		...
		int exitCode = -1;
		String[] command = {"sh", "-c",
				String.format(MlflowConstants.DOCKER_HEALTH_CHECK, mlflowParameters.getContainerName())};
