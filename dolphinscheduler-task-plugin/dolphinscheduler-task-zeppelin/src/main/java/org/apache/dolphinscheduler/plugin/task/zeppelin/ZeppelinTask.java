...
public class ZeppelinTask extends AbstractRemoteTask {
	...
	public void init() {
		...
		zeppelinTaskExecutionContext = zeppelinParameters
				.generateExtendedContext(taskExecutionContext.getResourceParametersHelper());
		zeppelinConnectionParam = (ZeppelinConnectionParam) DataSourceUtils.buildConnectionParams(
				DbType.valueOf(zeppelinParameters.getType()), zeppelinTaskExecutionContext.getConnectionParams());
	...
	public void handle(TaskCallBack taskCallBack) throws TaskException {
		try {
			...
			if (productionNoteDirectory != null) {
				final String cloneNotePath = String.format("%s%s_%s", productionNoteDirectory, noteId,
			...
			if (paragraphId == null || paragraphId.trim().length() == 0) {
				...
				final ParagraphResult paragraphResult = this.zClient.executeParagraph(noteId, paragraphId,
						zeppelinParamsMap);
