...
public class ProcedureTask extends AbstractTask {
	...
	public ProcedureTask(TaskExecutionContext taskExecutionContext) {
		...
		this.procedureParameters = JSONUtils.parseObject(taskExecutionContext.getTaskParams(),
		...
		procedureTaskExecutionContext = procedureParameters
				.generateExtendedContext(taskExecutionContext.getResourceParametersHelper());
	}

	@Override
	public void handle(TaskCallBack taskCallBack) throws TaskException {
		log.info("procedure type : {}, datasource : {}, method : {} , localParams : {}", procedureParameters.getType(),
				procedureParameters.getDatasource(), procedureParameters.getMethod(),
		...
		ConnectionParam connectionParams = dataSourceProcessor
				.createConnectionParams(procedureTaskExecutionContext.getConnectionParams());
		try (Connection connection = DataSourceClientProvider.getAdHocConnection(dbType, connectionParams)) {
			Map<Integer, Property> sqlParamsMap = new HashMap<>();
			Map<String, Property> paramsMap = taskExecutionContext.getPrepareParamsMap() == null
					? Maps.newHashMap()
	 ...
	 *             SQLException
	 */
	private Object getOutputParameter(CallableStatement stmt, int index, String prop, DataType dataType)
			throws SQLException {
		Object value = null;
		switch (dataType) {
			case VARCHAR :
