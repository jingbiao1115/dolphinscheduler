...
public class DatasyncHook {
	...
	public static TaskExecutionStatus[] doneStatus = {TaskExecutionStatus.ERROR, TaskExecutionStatus.SUCCESS,
	...
	public Boolean createDatasyncTask(DatasyncParameters parameters) {
		...
		CreateTaskRequest.Builder builder = CreateTaskRequest.builder().name(parameters.getName())
	...
	public TaskExecutionStatus queryDatasyncTaskExecStatus() {
		...
		DescribeTaskExecutionRequest request = DescribeTaskExecutionRequest.builder().taskExecutionArn(taskExecArn)
				.build();
	...
	private static void castParamPropertyPackage(DatasyncParameters parameters, CreateTaskRequest.Builder builder) {
		...
		if (tags != null && tags.size() > 0) {
			List<TagListEntry> collect = tags.stream()
					.map(e -> TagListEntry.builder().key(e.getKey()).value(e.getValue()).build())
