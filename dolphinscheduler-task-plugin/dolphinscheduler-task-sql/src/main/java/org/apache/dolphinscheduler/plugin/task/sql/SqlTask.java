...
public class SqlTask extends AbstractTask {
	...
	/**
	 * create function format include replace here which can be compatible with more
	 * cases, for example a long-running Spark session in Kyuubi will keep its own
	 * temp functions instead of destroying them right away
	 */
	...
	public SqlTask(TaskExecutionContext taskRequest) {
		...
		sqlTaskExecutionContext = sqlParameters
				.generateExtendedContext(taskExecutionContext.getResourceParametersHelper());
	...
	public void handle(TaskCallBack taskCallBack) throws TaskException {
		...
		log.info(
				...
				sqlParameters.getLocalParams(), sqlParameters.getUdfs(), sqlParameters.getShowType(),
				sqlParameters.getConnParams(), sqlParameters.getVarPool(), sqlParameters.getLimit());
		try {
			...
			List<SqlBinds> mainStatementSqlBinds = subSqls.stream().map(this::getSqlAndSqlParamsMap)
					.collect(Collectors.toList());

			List<SqlBinds> preStatementSqlBinds = Optional.ofNullable(sqlParameters.getPreStatements())
					.orElse(new ArrayList<>()).stream().map(this::getSqlAndSqlParamsMap).collect(Collectors.toList());
	 ...
	 *
	 * @param mainStatementsBinds
	 *            main statements binds
	 * @param preStatementsBinds
	 *            pre statements binds
	 * @param postStatementsBinds
	 *            post statements binds
	 * @param createFuncs
	 *            create functions
	 */
	public void executeFuncAndSql(List<SqlBinds> mainStatementsBinds, List<SqlBinds> preStatementsBinds,
			List<SqlBinds> postStatementsBinds, List<String> createFuncs) throws Exception {
		try (Connection connection = DataSourceClientProvider
				.getAdHocConnection(DbType.valueOf(sqlParameters.getType()), baseConnectionParam)) {
			...
			if (sqlParameters.getSqlType() == SqlType.QUERY.ordinal()) {
				// query statements need to be convert to JsonArray and inserted into Alert to
				// send
	...
	private String executeUpdate(Connection connection, List<SqlBinds> statementsBinds, String handlerType)
		...
		for (SqlBinds sqlBind : statementsBinds) {
			try (PreparedStatement statement = prepareStatementAndBind(connection, sqlBind)) {
				...
				log.info("{} statement execute update result: {}, for sql: {}", handlerType, result, sqlBind.getSql());
			}
		}
	...
	private List<String> buildTempFuncSql(List<UdfFuncParameters> udfFuncParameters) {
		return udfFuncParameters.stream().map(value -> MessageFormat.format(CREATE_OR_REPLACE_FUNCTION_FORMAT,
