...
public class SqlServerSourceGenerator implements ISourceGenerator {
	...
	public String generate(SqoopParameters sqoopParameters, SqoopTaskExecutionContext sqoopTaskExecutionContext) {
		...
		try {
			SourceSqlServerParameter sourceSqlServerParameter = JSONUtils.parseObject(sqoopParameters.getSourceParams(),
					SourceSqlServerParameter.class);
			...
			sqlServerSourceSb.append(SPACE).append(DB_CONNECT).append(SPACE).append(DOUBLE_QUOTES)
					.append(DataSourceUtils.getJdbcUrl(DbType.SQLSERVER, baseDataSource)).append(DOUBLE_QUOTES)
					.append(SPACE).append(DRIVER).append(SPACE)
					.append(DataSourceUtils.getDatasourceDriver(DbType.SQLSERVER)).append(SPACE).append(DB_USERNAME)
					.append(SPACE).append(baseDataSource.getUser()).append(SPACE).append(DB_PWD).append(SPACE)
					.append(DOUBLE_QUOTES).append(decodePassword(baseDataSource.getPassword())).append(DOUBLE_QUOTES);
			...
			if (sourceSqlServerParameter.getSrcQueryType() == SqoopQueryType.FORM.getCode()) {
				if (StringUtils.isNotEmpty(sourceSqlServerParameter.getSrcTable())) {
					sqlServerSourceSb.append(SPACE).append(TABLE).append(SPACE)
							.append(sourceSqlServerParameter.getSrcTable());
				}

				if (StringUtils.isNotEmpty(sourceSqlServerParameter.getSrcColumns())) {
					sqlServerSourceSb.append(SPACE).append(COLUMNS).append(SPACE)
							.append(sourceSqlServerParameter.getSrcColumns());
				}
				...
				sqlServerSourceSb.append(SPACE).append(QUERY).append(SPACE).append(DOUBLE_QUOTES).append(srcQuery);
	...
	private static void buildColumnMapToHive(StringBuilder sqlServerSourceSb,
		...
		if (null != mapColumnHive && !mapColumnHive.isEmpty()) {
			...
			if (StringUtils.isNotEmpty(columnMap.toString())) {
				sqlServerSourceSb.append(SPACE).append(MAP_COLUMN_HIVE).append(SPACE)
	...
	private static void buildColumnMapToJava(StringBuilder sqlServerSourceSb,
		...
		if (null != mapColumnJava && !mapColumnJava.isEmpty()) {
			...
			if (StringUtils.isNotEmpty(columnMap.toString())) {
				sqlServerSourceSb.append(SPACE).append(MAP_COLUMN_JAVA).append(SPACE)
						.append(columnMap.substring(0, columnMap.length() - 1));
			}
