...
public class CommonGenerator {
	...
	public String generate(SqoopParameters sqoopParameters) {
		...
		try {
			// sqoop task model
			commonSb.append(SqoopConstants.SQOOP).append(SPACE).append(sqoopParameters.getModelType());

			// sqoop sqoop.export.records.per.statement
			commonSb.append(SPACE).append(D).append(SPACE).append(
					String.format(FORMAT_S_S_S, SqoopConstants.SQOOP_EXPORT_RECORDS_PER_STATEMENT, EQUAL_SIGN, 1));

			// sqoop map-reduce job name
			commonSb.append(SPACE).append(D).append(SPACE).append(String.format(FORMAT_S_S_S,
					SqoopConstants.SQOOP_MR_JOB_NAME, EQUAL_SIGN, sqoopParameters.getJobName()));
			...
			if (CollectionUtils.isNotEmpty(hadoopCustomParams)) {
				for (Property hadoopCustomParam : hadoopCustomParams) {
					String hadoopCustomParamStr = String.format(FORMAT_S_S_S, hadoopCustomParam.getProp(), EQUAL_SIGN,
							hadoopCustomParam.getValue());

					commonSb.append(SPACE).append(D).append(SPACE).append(hadoopCustomParamStr);
				}
			}
			...
			if (CollectionUtils.isNotEmpty(sqoopAdvancedParams)) {
				for (Property sqoopAdvancedParam : sqoopAdvancedParams) {
					commonSb.append(SPACE).append(sqoopAdvancedParam.getProp()).append(SPACE)
							.append(sqoopAdvancedParam.getValue());
				}
			}

			// sqoop parallelism
			if (sqoopParameters.getConcurrency() > 0) {
				commonSb.append(SPACE).append(SqoopConstants.SQOOP_PARALLELISM).append(SPACE)
						.append(sqoopParameters.getConcurrency());
				if (sqoopParameters.getConcurrency() > 1) {
					commonSb.append(SPACE).append(SqoopConstants.SPLIT_BY).append(SPACE)
							.append(sqoopParameters.getSplitBy());
				}
			}
		} catch (Exception e) {
