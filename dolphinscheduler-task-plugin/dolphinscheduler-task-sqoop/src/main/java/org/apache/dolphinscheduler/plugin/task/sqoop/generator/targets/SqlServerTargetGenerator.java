...
public class SqlServerTargetGenerator implements ITargetGenerator {
	...
	public String generate(SqoopParameters sqoopParameters, SqoopTaskExecutionContext sqoopTaskExecutionContext) {
		...
		try {
			TargetSqlServerParameter targetSqlServerParameter = JSONUtils.parseObject(sqoopParameters.getTargetParams(),
					TargetSqlServerParameter.class);
			...
			oracleTargetSb.append(SPACE).append(DB_CONNECT).append(SPACE).append(DOUBLE_QUOTES)
					.append(DataSourceUtils.getJdbcUrl(DbType.SQLSERVER, baseDataSource)).append(DOUBLE_QUOTES)
					// .append(SPACE).append(DRIVER)
					.append(SPACE).append(DataSourceUtils.getDatasourceDriver(DbType.SQLSERVER)).append(SPACE)
					...
					.append(targetSqlServerParameter.getTargetTable());

			if (StringUtils.isNotEmpty(targetSqlServerParameter.getTargetColumns())) {
				oracleTargetSb.append(SPACE).append(COLUMNS).append(SPACE)
						.append(targetSqlServerParameter.getTargetColumns());
			}
			...
			if (targetSqlServerParameter.getIsUpdate()
				...
				oracleTargetSb.append(SPACE).append(UPDATE_KEY).append(SPACE)
						.append(targetSqlServerParameter.getTargetUpdateKey()).append(SPACE).append(UPDATE_MODE)
