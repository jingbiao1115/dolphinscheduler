...
public class HiveTargetGenerator implements ITargetGenerator {
	...
	public String generate(SqoopParameters sqoopParameters, SqoopTaskExecutionContext sqoopTaskExecutionContext) {
		...
		try {
			TargetHiveParameter targetHiveParameter = JSONUtils.parseObject(sqoopParameters.getTargetParams(),
			...
			if (null != targetHiveParameter) {
				...
				if (StringUtils.isNotEmpty(targetHiveParameter.getHiveDatabase())
					...
					hiveTargetSb.append(SPACE).append(HIVE_DATABASE).append(SPACE)
							.append(targetHiveParameter.getHiveDatabase()).append(SPACE).append(HIVE_TABLE)
				...
				if (targetHiveParameter.isHiveOverWrite()) {
					hiveTargetSb.append(SPACE).append(HIVE_OVERWRITE).append(SPACE).append(DELETE_TARGET_DIR);
				}

				if (StringUtils.isNotEmpty(targetHiveParameter.getReplaceDelimiter())) {
					hiveTargetSb.append(SPACE).append(HIVE_DELIMS_REPLACEMENT).append(SPACE)
							.append(targetHiveParameter.getReplaceDelimiter());
				}
				...
				if (StringUtils.isNotEmpty(targetHiveParameter.getHivePartitionKey())
					...
					hiveTargetSb.append(SPACE).append(HIVE_PARTITION_KEY).append(SPACE)
							.append(targetHiveParameter.getHivePartitionKey()).append(SPACE)
							.append(HIVE_PARTITION_VALUE).append(SPACE)
							.append(targetHiveParameter.getHivePartitionValue());
				}

				if (StringUtils.isNotEmpty(targetHiveParameter.getHiveTargetDir())) {
					hiveTargetSb.append(SPACE).append(TARGET_DIR).append(SPACE)
							.append(targetHiveParameter.getHiveTargetDir());
				}
