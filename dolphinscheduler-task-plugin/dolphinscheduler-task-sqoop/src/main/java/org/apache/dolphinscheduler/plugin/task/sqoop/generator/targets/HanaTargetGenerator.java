...
public class HanaTargetGenerator implements ITargetGenerator {
	...
	public String generate(SqoopParameters sqoopParameters, SqoopTaskExecutionContext sqoopTaskExecutionContext) {
		...
		try {
			TargetHanaParameter targetHanaParameter = JSONUtils.parseObject(sqoopParameters.getTargetParams(),
					TargetHanaParameter.class);
			...
			BaseConnectionParam baseDataSource = (BaseConnectionParam) DataSourceUtils.buildConnectionParams(
					sqoopTaskExecutionContext.getTargetType(), sqoopTaskExecutionContext.getTargetConnectionParams());

			if (null == baseDataSource) {
				return hanaTargetSb.toString();
			}
			hanaTargetSb.append(SPACE).append(DB_CONNECT).append(SPACE).append(DOUBLE_QUOTES)
					.append(DataSourceUtils.getJdbcUrl(DbType.HANA, baseDataSource)).append(DOUBLE_QUOTES).append(SPACE)
					.append(DRIVER).append(SPACE).append(DataSourceUtils.getDatasourceDriver(DbType.HANA)).append(SPACE)
					.append(DB_USERNAME).append(SPACE).append(baseDataSource.getUser()).append(SPACE).append(DB_PWD)
					.append(SPACE).append(DOUBLE_QUOTES).append(decodePassword(baseDataSource.getPassword()))
					.append(DOUBLE_QUOTES).append(SPACE).append(TABLE).append(SPACE)
					.append(targetHanaParameter.getTargetTable());

			if (StringUtils.isNotEmpty(targetHanaParameter.getTargetColumns())) {
				hanaTargetSb.append(SPACE).append(COLUMNS).append(SPACE).append(targetHanaParameter.getTargetColumns());
			}
			...
			if (StringUtils.isNotEmpty(targetHanaParameter.getFieldsTerminated())) {
				...
				if (targetHanaParameter.getFieldsTerminated().contains("'")) {
					...
					hanaTargetSb.append(SPACE).append(SINGLE_QUOTES).append(targetHanaParameter.getFieldsTerminated())
			...
			if (StringUtils.isNotEmpty(targetHanaParameter.getLinesTerminated())) {
				...
				if (targetHanaParameter.getLinesTerminated().contains(SINGLE_QUOTES)) {
					...
					hanaTargetSb.append(SPACE).append(SINGLE_QUOTES).append(targetHanaParameter.getLinesTerminated())
							.append(SINGLE_QUOTES);
				}
			}

			if (targetHanaParameter.getIsUpdate() && StringUtils.isNotEmpty(targetHanaParameter.getTargetUpdateKey())
					&& StringUtils.isNotEmpty(targetHanaParameter.getTargetUpdateMode())) {
				hanaTargetSb.append(SPACE).append(UPDATE_KEY).append(SPACE)
						.append(targetHanaParameter.getTargetUpdateKey()).append(SPACE).append(UPDATE_MODE)
