		...
		try {
			...
			if (null != targetMysqlParameter && targetMysqlParameter.getTargetDatasource() != 0) {
				...
				if (null != baseDataSource) {
					...
					mysqlTargetSb.append(SPACE).append(DB_CONNECT).append(SPACE).append(DOUBLE_QUOTES)
							.append(DataSourceUtils.getJdbcUrl(DbType.MYSQL, baseDataSource)).append(DOUBLE_QUOTES)
							.append(SPACE).append(DB_USERNAME).append(SPACE).append(baseDataSource.getUser())
							.append(SPACE).append(DB_PWD).append(SPACE).append(DOUBLE_QUOTES)
							.append(decodePassword(baseDataSource.getPassword())).append(DOUBLE_QUOTES).append(SPACE)
							.append(TABLE).append(SPACE).append(targetMysqlParameter.getTargetTable());

					if (StringUtils.isNotEmpty(targetMysqlParameter.getTargetColumns())) {
						mysqlTargetSb.append(SPACE).append(COLUMNS).append(SPACE)
								.append(targetMysqlParameter.getTargetColumns());
					}
					...
					if (targetMysqlParameter.getIsUpdate()
						...
						mysqlTargetSb.append(SPACE).append(UPDATE_KEY).append(SPACE)
								.append(targetMysqlParameter.getTargetUpdateKey()).append(SPACE).append(UPDATE_MODE)
