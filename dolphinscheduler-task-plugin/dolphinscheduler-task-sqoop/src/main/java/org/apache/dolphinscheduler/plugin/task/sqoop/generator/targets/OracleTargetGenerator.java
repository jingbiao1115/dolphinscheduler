...
public class OracleTargetGenerator implements ITargetGenerator {
	...
	public String generate(SqoopParameters sqoopParameters, SqoopTaskExecutionContext sqoopTaskExecutionContext) {
		...
		try {
			TargetOracleParameter targetOracleParameter = JSONUtils.parseObject(sqoopParameters.getTargetParams(),
					TargetOracleParameter.class);
			...
			if (targetOracleParameter.getIsUpdate()
				...
				oracleTargetSb.append(SPACE).append(UPDATE_KEY).append(SPACE)
						.append(targetOracleParameter.getTargetUpdateKey()).append(SPACE).append(UPDATE_MODE)
