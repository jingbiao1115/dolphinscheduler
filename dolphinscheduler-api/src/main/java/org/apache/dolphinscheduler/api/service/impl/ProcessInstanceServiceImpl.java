...
public class ProcessInstanceServiceImpl extends BaseServiceImpl implements ProcessInstanceService {
	...
	/**
	 * return top n SUCCESS process instance order by running time which started
	...
	/**
	 * paging query process instance list, filtering according to project, process
	 * definition, time range, keyword, process status
	 *
	 * @param loginUser
	 *            login user
	 * @param projectCode
	 ...
	 */
	@Override
	public Result<PageInfo<ProcessInstance>> queryProcessInstanceList(User loginUser, long projectCode,
			long processDefineCode, String startDate, String endDate, String searchVal, String executorName,
			WorkflowExecutionStatus stateType, String host, String otherParamsJson, Integer pageNo, Integer pageSize) {
		...
		IPage<ProcessInstance> processInstanceList = processInstanceMapper.queryProcessInstanceListPaging(page,
				projectCode, processDefineCode, searchVal, executorName, statusArray, host, start, end);
		...
		int total = (int) processInstanceList.getTotal();
	 ...
	 * @param loginUser
	 *            login user
	 * @param workflowInstanceQueryRequest
	...
	public Result queryProcessInstanceList(User loginUser, WorkflowInstanceQueryRequest workflowInstanceQueryRequest) {
		...
		if (!StringUtils.isBlank(projectName)) {
			...
			ProcessDefinition processDefinition = processDefineMapper.queryByDefineName(project.getCode(),
					processInstance.getName());
			processInstance.setProcessDefinitionCode(processDefinition.getCode());
			processInstance.setProjectCode(project.getCode());
		}

		Page<ProcessInstance> page = new Page<>(workflowInstanceQueryRequest.getPageNo(),
				workflowInstanceQueryRequest.getPageSize());
		PageInfo<ProcessInstance> pageInfo = new PageInfo<>(workflowInstanceQueryRequest.getPageNo(),
				workflowInstanceQueryRequest.getPageSize());

		IPage<ProcessInstance> processInstanceList = processInstanceMapper.queryProcessInstanceListV2Paging(page,
				processInstance.getProjectCode(), processInstance.getProcessDefinitionCode(), processInstance.getName(),
	 ...
	 */
	@Override
	public Map<String, Object> queryTaskListByProcessId(User loginUser, long projectCode, Integer processId)
			throws IOException {
		Project project = projectMapper.queryByCode(projectCode);
		// check user access for project
		Map<String, Object> result = projectService.checkProjectAndAuth(loginUser, project, projectCode,
		...
		List<TaskInstance> taskInstanceList = taskInstanceDao.queryValidTaskListByWorkflowInstanceId(processId,
	...
	public List<DynamicSubWorkflowDto> queryDynamicSubWorkflowInstances(User loginUser, Integer taskId) {
		...
		List<RelationSubWorkflow> relationSubWorkflows = relationSubWorkflowMapper
				.queryAllSubProcessInstance((long) taskInstance.getProcessInstanceId(), taskInstance.getTaskCode());
		...
		ProcessDefinition subProcessDefinition = processService.findProcessDefinition(subWorkflowCode,
				subWorkflowVersion);
	...
	private void addDependResultForTaskList(User loginUser, List<TaskInstance> taskInstanceList) throws IOException {
		for (TaskInstance taskInstance : taskInstanceList) {
			if (TASK_TYPE_DEPENDENT.equalsIgnoreCase(taskInstance.getTaskType())) {
				...
				// TODO The result of dependent item should not be obtained from the log,
				// waiting for optimization.
				Result<ResponseTaskLog> logResult = loggerService.queryLog(loginUser, taskInstance.getId(),
	...
	public Map<String, Object> querySubProcessInstanceByTaskId(User loginUser, long projectCode, Integer taskId) {
		...
		if (!taskInstance.isSubProcess()) {
			log.warn("Task instance is not {} type instance, projectCode:{}, taskInstanceId:{}.", TASK_TYPE_SUB_PROCESS,
					projectCode, taskId);
			putMsg(result, Status.TASK_INSTANCE_NOT_SUB_WORKFLOW_INSTANCE, taskInstance.getName());
			return result;
		}

		ProcessInstance subWorkflowInstance = processService.findSubProcessInstance(taskInstance.getProcessInstanceId(),
				taskInstance.getId());
	...
	public Map<String, Object> updateProcessInstance(User loginUser, long projectCode, Integer processInstanceId,
			String taskRelationJson, String taskDefinitionJson, String scheduleTime, Boolean syncDefine,
			String globalParams, String locations, int timeout) {
		// check user access for project
		...
		ProcessDefinition processDefinition0 = processDefineMapper
				.queryByCode(processInstance.getProcessDefinitionCode());
		...
		if (!processInstance.getState().isFinished()) {
			...
			putMsg(result, PROCESS_INSTANCE_STATE_OPERATION_ERROR, processInstance.getName(),
	 ...
	 */
	private void setProcessInstance(ProcessInstance processInstance, String scheduleTime, String globalParams,
			int timeout, String timezone) {
		...
		Map<String, String> globalParamMap = globalParamList.stream()
	...
	public Map<String, Object> queryParentInstanceBySubId(User loginUser, long projectCode, Integer subId) {
		...
		if (subInstance.getIsSubProcess() == Flag.NO) {
			log.warn("Process instance is not sub process instance type, processInstanceId:{}, processInstanceName:{}.",
		...
		if (parentWorkflowInstance == null) {
			log.error("Parent process instance does not exist, projectCode:{}, subProcessInstanceId:{}.", projectCode,
					subId);
	...
	/**
	 * delete process instance by id, at the same time，delete task instance and
	 * their mapping relation data
	 *
	 * @param loginUser
	 *            login user
	 * @param processInstanceId
	...
	public Map<String, Object> viewVariables(long projectCode, Integer processInstanceId) {
		...
		// global param string
		String globalParamStr = ParameterUtils.convertParameterPlaceholders(JSONUtils.toJsonString(globalParams),
				timeParams);
	...
	private Map<String, Map<String, Object>> getLocalParams(ProcessInstance processInstance,
		...
		List<TaskInstance> taskInstanceList = taskInstanceMapper.findValidTaskListByProcessId(processInstance.getId(),
				Flag.YES, processInstance.getTestFlag());
		for (TaskInstance taskInstance : taskInstanceList) {
	...
	public Map<String, Object> viewGantt(long projectCode, Integer processInstanceId) throws Exception {
		...
		if (CollectionUtils.isNotEmpty(nodeList)) {
			List<TaskInstance> taskInstances = taskInstanceMapper
					.queryByProcessInstanceIdsAndTaskCodes(Collections.singletonList(processInstanceId), nodeList);
	...
	public void deleteProcessInstanceByWorkflowDefinitionCode(long workflowDefinitionCode) {
		while (true) {
			List<ProcessInstance> processInstances = processInstanceMapper
					.queryByProcessDefineCode(workflowDefinitionCode, 100);
			...
			log.info("Success delete workflow instance, workflow definition code: {}, size: {}", workflowDefinitionCode,
