...
public class ProjectParameterServiceImpl extends BaseServiceImpl implements ProjectParameterService {
	...
	public Result createProjectParameter(User loginUser, long projectCode, String projectParameterName,
		...
		ProjectParameter projectParameter = projectParameterMapper.selectOne(
				new QueryWrapper<ProjectParameter>().lambda().eq(ProjectParameter::getProjectCode, projectCode)
		...
		try {
			projectParameter = ProjectParameter.builder().paramName(projectParameterName)
					.paramValue(projectParameterValue).code(CodeGenerateUtils.getInstance().genCode())
					.projectCode(projectCode).userId(loginUser.getId()).createTime(now).updateTime(now).build();
	...
	public Result updateProjectParameter(User loginUser, long projectCode, long code, String projectParameterName,
		...
		// check if project parameter name exists
		ProjectParameter tempProjectParameter = projectParameterMapper.selectOne(
				new QueryWrapper<ProjectParameter>().lambda().eq(ProjectParameter::getProjectCode, projectCode)
						.eq(ProjectParameter::getParamName, projectParameterName).ne(ProjectParameter::getCode, code));
