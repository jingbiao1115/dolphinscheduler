...
public class ProcessTaskRelationServiceImpl extends BaseServiceImpl implements ProcessTaskRelationService {
	...
	public Map<String, Object> createProcessTaskRelation(User loginUser, long projectCode, long processDefinitionCode,
		...
		if (!processTaskRelations.isEmpty()) {
			Map<Long, ProcessTaskRelation> preTaskCodeMap = processTaskRelations.stream()
					.filter(r -> r.getPostTaskCode() == postTaskCode).collect(Collectors
							.toMap(ProcessTaskRelation::getPreTaskCode, processTaskRelation -> processTaskRelation));
	...
	private void updateVersions(ProcessTaskRelation processTaskRelation) {
		...
		ProcessDefinition processDefinition = processDefinitionMapper
				.queryByCode(processTaskRelation.getProcessDefinitionCode());
	 ...
	 * @param loginUser
	 *            login user
	 * @param taskRelationCreateRequest
	 *            project code
	...
	private void updateProcessDefiniteVersion(User loginUser, Map<String, Object> result,
		...
		if (insertVersion <= 0) {
			...
			log.info("Update process definition complete, new version is {}, projectCode:{}, processDefinitionCode:{}.",
	...
	/**
	 * delete process task relation, will delete exists relation preTaskCode ->
	 * postTaskCode, throw error if not exists
	 *
	 * @param loginUser
	 *            login user
	 * @param preTaskCode
	 *            relation upstream code
	 * @param postTaskCode
	 *            relation downstream code
	 */
	@Override
	@Transactional
	public void deleteTaskProcessRelationV2(User loginUser, long preTaskCode, long postTaskCode) {
		ProcessTaskRelation processTaskRelation = new TaskRelationFilterRequest(preTaskCode, postTaskCode)
				.convert2TaskDefinition();

		Page<ProcessTaskRelation> page = new Page<>(
				new TaskRelationFilterRequest(preTaskCode, postTaskCode).getPageNo(),
				new TaskRelationFilterRequest(preTaskCode, postTaskCode).getPageSize());
		IPage<ProcessTaskRelation> processTaskRelationIPage = processTaskRelationMapper.filterProcessTaskRelation(page,
				processTaskRelation);
	...
	/**
	 * delete process task relation, will delete exists relation upstream ->
	 * downstream, throw error if not exists
	 *
	 * @param loginUser
	 *            login user
	 * @param taskCode
	 ...
	 * @param needSyncDag
	 *            needSyncDag
	 * @param taskRelationUpdateUpstreamRequest
	 *            relation downstream code
	 */
	@Override
	@Transactional
	public List<ProcessTaskRelation> updateUpstreamTaskDefinitionWithSyncDag(User loginUser, long taskCode,
			Boolean needSyncDag, TaskRelationUpdateUpstreamRequest taskRelationUpdateUpstreamRequest) {
		...
		IPage<ProcessTaskRelation> processTaskRelationExistsIPage = processTaskRelationMapper
				.filterProcessTaskRelation(page, processTaskRelation);
		...
		if (CollectionUtils.isNotEmpty(processTaskRelationExists)) {
			processDefinition = processDefinitionMapper
					.queryByCode(processTaskRelationExists.get(0).getProcessDefinitionCode());
		} else if (taskRelationUpdateUpstreamRequest.getWorkflowCode() != 0L) {
			processDefinition = processDefinitionMapper
					.queryByCode(taskRelationUpdateUpstreamRequest.getWorkflowCode());
		}
		if (processDefinition == null) {
		...
		if (needSyncDag) {
			insertVersion = this.saveProcessDefine(loginUser, processDefinition);
			if (insertVersion <= 0) {
				throw new ServiceException(Status.UPDATE_PROCESS_DEFINITION_ERROR);
			}
		}
		// get new relation to create and out of date relation to delete
		List<Long> taskCodeCreates = upstreamTaskCodes.stream()
		...
		List<Integer> taskCodeDeletes = processTaskRelationExists.stream()
				.filter(ptr -> !upstreamTaskCodes.contains(ptr.getPreTaskCode())).map(ProcessTaskRelation::getId)
				.collect(Collectors.toList());
		...
		for (long createCode : taskCodeCreates) {
			...
			ProcessTaskRelation processTaskRelationCreate = new ProcessTaskRelation(null,
					processDefinition.getVersion(), downstreamTask.getProjectCode(), processDefinition.getCode(),
					upstreamCode, version, downstreamTask.getCode(), downstreamTask.getVersion(), null, null);
	...
	private void updateRelation(User loginUser, Map<String, Object> result, ProcessDefinition processDefinition,
		...
		int insertResult = processService.saveTaskRelation(loginUser, processDefinition.getProjectCode(),
				processDefinition.getCode(), processDefinition.getVersion(), relationLogs, Lists.newArrayList(),
				Boolean.TRUE);
	...
	public Map<String, Object> deleteUpstreamRelation(User loginUser, long projectCode, String preTaskCodes,
		...
		List<Long> currentUpstreamList = upstreamList.stream().map(ProcessTaskRelation::getPreTaskCode)
				.collect(Collectors.toList());
		...
		ProcessDefinition processDefinition = processDefinitionMapper
				.queryByCode(upstreamList.get(0).getProcessDefinitionCode());
		if (processDefinition == null) {
	...
	public Map<String, Object> deleteDownstreamRelation(User loginUser, long projectCode, String postTaskCodes,
		...
		List<ProcessTaskRelation> downstreamList = processTaskRelationMapper.queryDownstreamByCode(projectCode,
				taskCode);
		...
		ProcessDefinition processDefinition = processDefinitionMapper
				.queryByCode(downstreamList.get(0).getProcessDefinitionCode());
		if (processDefinition == null) {
			log.error("Process definition does not exist, processDefinitionCode:{}.",
	 ...
	 *            project code
	 * @param taskCode
	 *            current task code (post task code)
	...
	public Map<String, Object> queryUpstreamRelation(User loginUser, long projectCode, long taskCode) {
		...
		List<ProcessTaskRelation> processTaskRelationList = processTaskRelationMapper.queryUpstreamByCode(projectCode,
				taskCode);
		List<TaskDefinitionLog> taskDefinitionLogList = new ArrayList<>();
		if (CollectionUtils.isNotEmpty(processTaskRelationList)) {
			Set<TaskDefinition> taskDefinitions = processTaskRelationList.stream().map(processTaskRelation -> {
	...
	public Map<String, Object> queryDownstreamRelation(User loginUser, long projectCode, long taskCode) {
		...
		List<ProcessTaskRelation> processTaskRelationList = processTaskRelationMapper.queryDownstreamByCode(projectCode,
				taskCode);
		List<TaskDefinitionLog> taskDefinitionLogList = new ArrayList<>();
		if (CollectionUtils.isNotEmpty(processTaskRelationList)) {
			Set<TaskDefinition> taskDefinitions = processTaskRelationList.stream().map(processTaskRelation -> {
	...
	private TaskDefinition buildTaskDefinition() {
		...
		return new TaskDefinition() {
			...
			public boolean equals(Object o) {
				...
				return getCode() == that.getCode() && getVersion() == that.getVersion()
