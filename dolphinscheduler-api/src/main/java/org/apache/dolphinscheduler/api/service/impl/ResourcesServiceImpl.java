...
public class ResourcesServiceImpl extends BaseServiceImpl implements ResourcesService {
	...
	private String getFullName(String currentDir, String name) {
		return currentDir.equals(FOLDER_SEPARATOR)
				? String.format(FORMAT_SS, currentDir, name)
	...
	@Override
	@Transactional
	public Result<Object> createResource(User loginUser, String name, ResourceType type, MultipartFile file,
			String currentDir) {
		Result<Object> result = new Result<>();
	 ...
	 *            resource full name
	 * @param resTenantCode
	 *            tenantCode in the request field "resTenantCode" for tenant code
	 *            owning the resource, can be different from the login user in the
	...
	public Result<PageInfo<StorageEntity>> queryResourceListPaging(User loginUser, String fullName,
			String resTenantCode, ResourceType type, String searchVal, Integer pageNo, Integer pageSize) {
		...
		int total = filteredResourceList.size();
		...
		if (isAdmin(loginUser) && StringUtils.isBlank(fullName)) {
			...
			try {
				...
				resourcesList = recursive
						? storageOperate.listFilesStatusRecursively(fullName, defaultPath, tenantCode, type)
	...
	/**
	 * create directory xxx The steps to verify resources are cumbersome and can be
	 * optimized
	 *
	 * @param loginUser
	 *            login user
	...
	public Result<Object> delete(User loginUser, String fullName, String resTenantCode) throws IOException {
		...
		List<String> allChildren = storageOperate
				.listFilesStatusRecursively(fullName, defaultPath, resTenantCode, resource.getType()).stream()
				.map(storageEntity -> storageEntity.getFullName()).collect(Collectors.toList());
	...
	private String RemoveResourceFromResourceList(String stringToDelete, String taskParameter, boolean isDir) {
		Map<String, Object> taskParameters = JSONUtils.parseObject(taskParameter,
		...
		if (taskParameters.containsKey("resourceList")) {
			...
			if (isDir) {
				...
				updatedResourceInfoList = resourceInfoList.stream().filter(Objects::nonNull)
	 ...
	 *            resource fullName
	 * @param resTenantCode
	 *            owner's tenant code of the resource
	 * @param skipLineNum
	 *            skip line number
	 * @param limit
	 *            limit
	...
	@Override
	@Transactional
	public StorageEntity createOrUpdateResource(String userName, String filepath, String resourceContent)
			throws Exception {
	 ...
	 * @return isValid
	 */
	private boolean isUserTenantValid(boolean isAdmin, String userTenantCode, String resTenantCode)
