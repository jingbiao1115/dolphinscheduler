...
public class RepeatRunningExecuteFunction implements ExecuteFunction<RepeatRunningRequest, RepeatRunningResult> {
	...
	public RepeatRunningResult execute(RepeatRunningRequest request) throws ExecuteRuntimeException {
		...
		// todo: check workflow definition valid? or we don't need to do this check,
		// since we will check in master
		// again.
		// todo: check tenant valid? or we don't need to do this check, since we need to
		// check in master again.
		...
		Command command = Command.builder().commandType(CommandType.REPEAT_RUNNING)
	...
	private Map<String, Object> createCommandParams(ProcessInstance workflowInstance) {
		Map<String, Object> commandMap = JSONUtils.parseObject(workflowInstance.getCommandParam(),
