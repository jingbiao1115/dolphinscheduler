...
public class TaskInstanceServiceImpl extends BaseServiceImpl implements TaskInstanceService {
	...
	/**
	 * query task list by project, process instance, task name, task start time,
	 * task end time, task status, keyword paging
	 *
	 * @param loginUser
	 *            login user
	 * @param projectCode
	 ...
	 */
	@Override
	public Result queryTaskListPaging(User loginUser, long projectCode, Integer processInstanceId,
			String processInstanceName, String processDefinitionName, String taskName, Long taskCode,
			String executorName, String startDate, String endDate, String searchVal, TaskExecutionStatus stateType,
			String host, TaskExecuteType taskExecuteType, Integer pageNo, Integer pageSize) {
		...
		if (taskExecuteType == TaskExecuteType.STREAM) {
			...
			taskInstanceIPage = taskInstanceMapper.queryStreamTaskInstanceListPaging(page, projectCode,
					processDefinitionName, searchVal, taskName, taskCode, executorName, statusArray, host,
					taskExecuteType, start, end);
		} else {
			taskInstanceIPage = taskInstanceMapper.queryTaskInstanceListPaging(page, projectCode, processInstanceId,
					processInstanceName, searchVal, taskName, taskCode, executorName, statusArray, host,
					taskExecuteType, start, end);
		}
		...
		List<Integer> executorIds = taskInstanceList.stream().map(TaskInstance::getExecutorId).distinct()
		...
		int total = (int) taskInstanceIPage.getTotal();
	...
	public Result forceTaskSuccess(User loginUser, long projectCode, Integer taskInstanceId) {
		...
		if (changedNum > 0) {
			...
			log.error("Task instance performs force success complete, projectCode:{}, taskInstanceId:{}", projectCode,
					taskInstanceId);
	...
	public Result taskSavePoint(User loginUser, long projectCode, Integer taskInstanceId) {
		...
		IStreamingTaskInstanceOperator streamingTaskInstanceOperator = SingletonJdkDynamicRpcClientProxyFactory
				.getProxyClient(taskInstance.getHost(), IStreamingTaskInstanceOperator.class);
		TaskInstanceTriggerSavepointResponse taskInstanceTriggerSavepointResponse = streamingTaskInstanceOperator
	...
	public Result stopTask(User loginUser, long projectCode, Integer taskInstanceId) {
		...
		TaskInstanceKillResponse taskInstanceKillResponse = iTaskInstanceOperator
	...
	public void deleteByWorkflowInstanceId(Integer workflowInstanceId) {
		List<TaskInstance> needToDeleteTaskInstances = taskInstanceDao.queryByWorkflowInstanceId(workflowInstanceId);
		...
		for (TaskInstance taskInstance : needToDeleteTaskInstances) {
			if (StringUtils.isNotBlank(taskInstance.getLogPath())) {
				ILogService iLogService = SingletonJdkDynamicRpcClientProxyFactory
						.getProxyClient(taskInstance.getHost(), ILogService.class);
