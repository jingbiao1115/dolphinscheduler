...
public class TaskDefinitionServiceImpl extends BaseServiceImpl implements TaskDefinitionService {
	...
	private void checkTaskDefinitionValid(User user, TaskDefinition taskDefinition, String permissions) {
		...
		if (!taskPluginManager.checkTaskParameters(ParametersNode.builder().taskType(taskDefinition.getTaskType())
				.taskParams(taskDefinition.getTaskParams()).dependence(taskDefinition.getDependence()).build())) {
	...
	public TaskDefinition createTaskDefinitionV2(User loginUser, TaskCreateRequest taskCreateRequest) {
		...
		// update related objects: task relationship, workflow's location(need to set to
		// null and front-end will auto
		// format it)
	 ...
	 *            task definition json object
	 * @param upstreamCodes
	 *            upstream task codes, sep comma
	 * @return create result code
	 */
	@Transactional
	@Override
	public Map<String, Object> createTaskBindsWorkFlow(User loginUser, long projectCode, long processDefinitionCode,
		...
		List<ProcessTaskRelationLog> processTaskRelationLogList = processTaskRelationMapper
				.queryByProcessCode(processDefinitionCode).stream().map(ProcessTaskRelationLog::new)
		...
		if (StringUtils.isNotBlank(upstreamCodes)) {
			...
			Set<Long> queryUpStreamTaskCodes = upstreamTaskDefinitionList.stream().map(TaskDefinition::getCode)
					.collect(Collectors.toSet());
		...
		int insertResult = processService.saveTaskRelation(loginUser, projectCode, processDefinition.getCode(),
				processDefinition.getVersion(), processTaskRelationLogList, Lists.newArrayList(), Boolean.TRUE);
	...
	private void taskCanDeleteValid(User user, TaskDefinition taskDefinition, User loginUser) {
		...
		List<ProcessTaskRelation> processTaskRelationList = processTaskRelationMapper
				.queryDownstreamByTaskCode(taskDefinition.getCode());
		if (CollectionUtils.isNotEmpty(processTaskRelationList)) {
			Set<Long> postTaskCodes = processTaskRelationList.stream().map(ProcessTaskRelation::getPostTaskCode)
					.collect(Collectors.toSet());
	 ...
	 *
	 * Only task release state offline and no downstream tasks can be deleted, will
	 * also remove the exists task relation [upstreamTaskCode, taskCode]
	 *
	 * @param loginUser
	 *            login user
	 * @param taskCode
	...
	public void deleteTaskDefinitionByCode(User loginUser, long taskCode) {
		...
		List<ProcessTaskRelation> taskRelationList = processTaskRelationMapper
				.queryUpstreamByCode(taskDefinition.getProjectCode(), taskCode);
		if (CollectionUtils.isNotEmpty(taskRelationList)) {
			log.debug("Task definition has upstream tasks, start handle them after delete task, taskDefinitionCode:{}.",
					taskCode);
	...
	public void updateDag(User loginUser, long processDefinitionCode, List<ProcessTaskRelation> processTaskRelationList,
		...
		List<ProcessTaskRelationLog> relationLogs = processTaskRelationList.stream().map(ProcessTaskRelationLog::new)
				.collect(Collectors.toList());
		int insertResult = processService.saveTaskRelation(loginUser, processDefinition.getProjectCode(),
				processDefinition.getCode(), insertVersion, relationLogs, taskDefinitionLogs, Boolean.TRUE);
		if (insertResult == Constants.EXIT_CODE_SUCCESS) {
			log.info(
	...
	public Map<String, Object> updateTaskDefinition(User loginUser, long projectCode, long taskCode,
		...
		Map<String, Object> result = new HashMap<>();
		TaskDefinitionLog taskDefinitionToUpdate = updateTask(loginUser, projectCode, taskCode, taskDefinitionJsonObj,
				result);
		if (taskDefinitionToUpdate == null) {
			return result;
		}
		List<ProcessTaskRelation> taskRelationList = processTaskRelationMapper.queryUpstreamByCode(projectCode,
				taskCode);
		if (CollectionUtils.isNotEmpty(taskRelationList)) {
			log.info("Task definition has upstream tasks, start handle them after update task, taskDefinitionCode:{}.",
					taskCode);
			long processDefinitionCode = taskRelationList.get(0).getProcessDefinitionCode();
			List<ProcessTaskRelation> processTaskRelations = processTaskRelationMapper
					.queryByProcessCode(processDefinitionCode);
	...
	public TaskDefinition updateTaskDefinitionV2(User loginUser, long taskCode, TaskUpdateRequest taskUpdateRequest) {
		...
		List<ProcessTaskRelation> taskRelationList = processTaskRelationMapper
				.queryUpstreamByCode(taskDefinitionUpdate.getProjectCode(), taskCode);
	 ...
	 * @param loginUser
	 *            login user
	 * @param taskFilterRequest
	 *            taskFilterRequest object you want to filter the resource task
	 *            definitions
	...
	public PageInfo<TaskDefinition> filterTaskDefinition(User loginUser, TaskFilterRequest taskFilterRequest) {
		...
		Page<TaskDefinition> page = new Page<>(taskFilterRequest.getPageNo(), taskFilterRequest.getPageSize());
		IPage<TaskDefinition> taskDefinitionIPage = taskDefinitionMapper.filterTaskDefinition(page, taskDefinition);

		int pageSize = taskFilterRequest.getPageSize();
		int total = (int) taskDefinitionIPage.getTotal();
		PageInfo<TaskDefinition> pageInfo = new PageInfo<>(taskFilterRequest.getPageNo(), pageSize);
	...
	private TaskDefinitionLog updateTask(User loginUser, long projectCode, long taskCode, String taskDefinitionJsonObj,
		...
		TaskDefinitionLog taskDefinitionToUpdate = JSONUtils.parseObject(taskDefinitionJsonObj,
		...
		if (!taskPluginManager.checkTaskParameters(ParametersNode.builder()
				.taskType(taskDefinitionToUpdate.getTaskType()).taskParams(taskDefinitionToUpdate.getTaskParams())
				.dependence(taskDefinitionToUpdate.getDependence()).build())) {
		...
		if (version == null || version == 0) {
			log.error("Max version task definitionLog can not be found in database, taskDefinitionCode:{}.", taskCode);
		...
		if (CollectionUtils.isNotEmpty(processTaskRelations)) {
			...
			for (Map.Entry<Long, List<ProcessTaskRelation>> processTaskRelationMap : processTaskRelationGroupList
				...
				int processDefinitionVersion = processDefinitionLogMapper
						.queryMaxVersionForDefinition(processDefinitionCode) + 1;
				...
				for (ProcessTaskRelation processTaskRelation : processTaskRelationList) {
					...
					int updateProcessDefinitionVersionCount = processTaskRelationMapper
							.updateProcessTaskRelationTaskVersion(processTaskRelation);
	...
	public Map<String, Object> updateTaskWithUpstream(User loginUser, long projectCode, long taskCode,
		...
		TaskDefinitionLog taskDefinitionToUpdate = updateTask(loginUser, projectCode, taskCode, taskDefinitionJsonObj,
				result);
		List<ProcessTaskRelation> upstreamTaskRelations = processTaskRelationMapper.queryUpstreamByCode(projectCode,
				taskCode);
		Set<Long> upstreamCodeSet = upstreamTaskRelations.stream().map(ProcessTaskRelation::getPreTaskCode)
				.collect(Collectors.toSet());
		...
		if (MapUtils.isNotEmpty(queryUpStreamTaskCodeMap)) {
			...
			List<ProcessTaskRelation> processTaskRelations = processTaskRelationMapper
					.queryByProcessCode(taskRelation.getProcessDefinitionCode());

			// set upstream code list
			updateUpstreamTask(new HashSet<>(queryUpStreamTaskCodeMap.keySet()), taskCode, projectCode,
	...
	private void updateUpstreamTask(Set<Long> allPreTaskCodeSet, long taskCode, long projectCode,
		...
		Map<Long, TaskDefinition> taskCodeMap = taskDefinitionList.stream()
				.collect(Collectors.toMap(TaskDefinition::getCode, Function.identity(), (a, b) -> a));
		...
		for (Long preTaskCode : addPreTaskSet) {
			...
			ProcessTaskRelation processTaskRelation = new ProcessTaskRelation(null, processDefinition.getVersion(),
					projectCode, processDefinition.getCode(), preTaskRelation.getCode(), preTaskRelation.getVersion(),
	...
	public Map<String, Object> switchVersion(User loginUser, long projectCode, long taskCode, int version) {
		...
		if (switchVersion > 0) {
			...
			if (CollectionUtils.isNotEmpty(taskRelationList)) {
				...
				log.info("Task definition version switch complete, switch task version to {}, taskDefinitionCode:{}.",
						version, taskCode);
				putMsg(result, Status.SUCCESS);
			}
		} else {
	...
	@Override
	public Result queryTaskDefinitionVersions(User loginUser, long projectCode, long taskCode, int pageNo,
			int pageSize) {
		...
		IPage<TaskDefinitionLog> taskDefinitionVersionsPaging = taskDefinitionLogMapper
				.queryTaskDefinitionVersionsPaging(page, taskCode, projectCode);
		List<TaskDefinitionLog> taskDefinitionLogs = taskDefinitionVersionsPaging.getRecords();

		int total = (int) taskDefinitionVersionsPaging.getTotal();
	...
	public Map<String, Object> deleteByCodeAndVersion(User loginUser, long projectCode, long taskCode, int version) {
		...
		if (taskDefinition == null) {
			...
			if (delete > 0) {
				log.info("Task definition version delete complete, projectCode:{}, taskDefinitionCode:{}, version:{}.",
						projectCode, taskCode, version);
				putMsg(result, Status.SUCCESS);
			} else {
	...
	public Map<String, Object> queryTaskDefinitionDetail(User loginUser, long projectCode, long taskCode) {
		...
		if (taskDefinition == null || projectCode != taskDefinition.getProjectCode()) {
			...
			List<ProcessTaskRelation> taskRelationList = processTaskRelationMapper.queryByCode(projectCode, 0, 0,
					taskCode);
			if (CollectionUtils.isNotEmpty(taskRelationList)) {
				taskRelationList = taskRelationList.stream().filter(v -> v.getPreTaskCode() != 0)
						.collect(Collectors.toList());
			}
	...
	@Override
	public Result queryTaskDefinitionListPaging(User loginUser, long projectCode, String searchTaskName,
			String taskType, TaskExecuteType taskExecuteType, Integer pageNo, Integer pageSize) {
		Result result = new Result();
		Project project = projectMapper.queryByCode(projectCode);
		// check user access for project
		Map<String, Object> checkResult = projectService.checkProjectAndAuth(loginUser, project, projectCode,
		...
		int total = (int) taskMainInfoIPage.getTotal();
		pageInfo.setTotal(total);
		pageInfo.setTotalPage((int) Math.ceil((double) total / pageSize));
	...
	private void fillRecords(long projectCode, IPage<TaskMainInfo> taskMainInfoIPage) {
		...
		if (CollectionUtils.isNotEmpty(taskMainInfoIPage.getRecords())) {
			...
			records = taskDefinitionMapper.queryDefineListByCodeList(projectCode, taskMainInfoIPage.getRecords()
					.stream().map(TaskMainInfo::getTaskCode).collect(Collectors.toList()));
		}
	...
	public Map<String, Object> releaseTaskDefinition(User loginUser, long projectCode, long code,
		...
		TaskDefinitionLog taskDefinitionLog = taskDefinitionLogMapper.queryByDefinitionCodeAndVersion(code,
				taskDefinition.getVersion());
		...
		switch (releaseState) {
			...
			case ONLINE :
				...
				if (StringUtils.isNotBlank(resourceIds)) {
					Integer[] resourceIdArray = Arrays.stream(resourceIds.split(",")).map(Integer::parseInt)
							.toArray(Integer[]::new);
		...
		log.error("Update taskDefinition state or taskDefinitionLog state to complete, taskDefinitionCode:{}.", code);
		putMsg(result, Status.SUCCESS);
		return result;
	}
