...
public class TaskGroupServiceImpl extends BaseServiceImpl implements TaskGroupService {
	...
	public Map<String, Object> createTaskGroup(User loginUser, Long projectCode, String name, String description,
		...
		Date now = new Date();
		TaskGroup taskGroup = TaskGroup.builder().name(name).projectCode(projectCode).description(description)
				.groupSize(groupSize).userId(loginUser.getId()).status(Flag.YES).createTime(now).updateTime(now)
				.build();
	...
	public Map<String, Object> updateTaskGroup(User loginUser, int id, String name, String description, int groupSize) {
		...
		Long exists = taskGroupMapper.selectCount(new QueryWrapper<TaskGroup>().lambda().eq(TaskGroup::getName, name)
				.eq(TaskGroup::getUserId, loginUser.getId()).ne(TaskGroup::getId, id));
	...
	public Map<String, Object> doQuery(User loginUser, int pageNo, int pageSize, int userId, String name,
		...
		Map<String, Object> result = new HashMap<>();
		Page<TaskGroup> page = new Page<>(pageNo, pageSize);
		IPage<TaskGroup> taskGroupPaging = taskGroupMapper.queryTaskGroupPaging(page, name, status);
