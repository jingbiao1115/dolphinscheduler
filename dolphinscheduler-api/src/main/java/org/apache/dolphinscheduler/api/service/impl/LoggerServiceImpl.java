...
public class LoggerServiceImpl extends BaseServiceImpl implements LoggerService {
	...
	private String queryLog(TaskInstance taskInstance, int skipLineNum, int limit) {
		...
		if (skipLineNum == 0) {
			String head = String.format(LOG_HEAD_FORMAT, logPath, taskInstance.getHost(),
		...
		try {
			TaskInstanceLogPageQueryRequest request = TaskInstanceLogPageQueryRequest.builder()
					.taskInstanceId(taskInstance.getId()).taskInstanceLogAbsolutePath(logPath).skipLineNum(skipLineNum)
					.limit(limit).build();
	...
	private byte[] getLogBytes(TaskInstance taskInstance) {
		...
		byte[] head = String.format(LOG_HEAD_FORMAT, logPath, host, Constants.SYSTEM_LINE_SEPARATOR)
		...
		ILogService iLogService = SingletonJdkDynamicRpcClientProxyFactory.getProxyClient(taskInstance.getHost(),
				ILogService.class);
		try {
			TaskInstanceLogFileDownloadRequest request = new TaskInstanceLogFileDownloadRequest(taskInstance.getId(),
					logPath);
