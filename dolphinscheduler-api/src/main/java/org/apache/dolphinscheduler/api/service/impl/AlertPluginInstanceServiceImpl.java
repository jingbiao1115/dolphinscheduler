...
public class AlertPluginInstanceServiceImpl extends BaseServiceImpl implements AlertPluginInstanceService {
	 ...
	 * @param loginUser
	 *            login user
	 * @param pluginDefineId
	 *            plugin define id
	 * @param instanceName
	 *            instance name
	 * @param pluginInstanceParams
	 *            plugin instance params
	 */
	@Override
	public AlertPluginInstance create(User loginUser, int pluginDefineId, String instanceName,
			AlertPluginInstanceType instanceType, WarningType warningType, String pluginInstanceParams) {
	...
	public AlertPluginInstance updateById(User loginUser, int pluginInstanceId, String instanceName,
		...
		String paramsMapJson = parsePluginParamsMap(pluginInstanceParams);
		AlertPluginInstance alertPluginInstance = new AlertPluginInstance(pluginInstanceId, paramsMapJson, instanceName,
				warningType, new Date());
	...
	public PageInfo<AlertPluginInstanceVO> listPaging(User loginUser, String searchVal, int pageNo, int pageSize) {
		...
		IPage<AlertPluginInstance> alertPluginInstanceIPage = alertPluginInstanceMapper
				.queryByInstanceNamePage(new Page<>(pageNo, pageSize), searchVal);

		PageInfo<AlertPluginInstanceVO> pageInfo = new PageInfo<>(pageNo, pageSize);
		int total = (int) alertPluginInstanceIPage.getTotal();
	...
	private List<AlertPluginInstanceVO> buildPluginInstanceVOList(List<AlertPluginInstance> alertPluginInstances) {
		...
		Map<Integer, PluginDefine> pluginDefineMap = pluginDefineList.stream()
				.collect(Collectors.toMap(PluginDefine::getId, Function.identity()));
		alertPluginInstances.forEach(alertPluginInstance -> {
			...
			// FIXME When the user removes the plug-in, this will happen. At this time,
			// maybe we should add a new field
	 ...
	 *
	 * @param pluginParams
	 *            Complete parameters(include ui)
	 ...
	 *
	 * @param pluginParamsMapString
	 *            k-v data
	 * @param pluginUiParams
	 *            Complete parameters(include ui)
	...
	private String parseToPluginUiParams(String pluginParamsMapString, String pluginUiParams) {
		List<Map<String, Object>> pluginParamsList = PluginParamsTransfer.generatePluginParams(pluginParamsMapString,
				pluginUiParams);
	...
	private boolean checkHasAssociatedAlertGroup(String id) {
		...
		Optional<String> first = idsList.stream().filter(k -> null != k && Arrays.asList(k.split(",")).contains(id))
				.findFirst();
