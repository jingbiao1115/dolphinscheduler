...
public class DqRuleServiceImpl extends BaseServiceImpl implements DqRuleService {
	...
	@Override
	public PageInfo<DqRule> queryRuleListPaging(User loginUser, String searchVal, Integer ruleType, String startTime,
		...
		IPage<DqRule> dqRulePage = dqRuleMapper.queryRuleListPaging(page, searchVal, ruleType, start, end);
		if (dqRulePage != null) {
			List<DqRule> dataList = dqRulePage.getRecords();
			dataList.forEach(dqRule -> {
				List<DqRuleInputEntry> ruleInputEntryList = DqRuleUtils
						.transformInputEntry(dqRuleInputEntryMapper.getRuleInputEntryList(dqRule.getId()));
	...
	private InputParam getTextareaParam(DqRuleInputEntry inputEntry) {
		...
		return InputParam.newBuilder(inputEntry.getField(), inputEntry.getTitle())
				.addValidate(Validate.newBuilder().setRequired(inputEntry.getIsValidate()).build()).setProps(paramProps)
				.setValue(inputEntry.getData()).setPlaceholder(inputEntry.getPlaceholder())
	...
	private SelectParam getSelectParam(DqRuleInputEntry inputEntry) {
		...
		switch (OptionSourceType.of(inputEntry.getOptionSourceType())) {
			...
			case COMPARISON_TYPE :
				...
				List<DqComparisonType> list = dqComparisonTypeMapper
						.selectList(new QueryWrapper<DqComparisonType>().orderByAsc("id"));
		...
		return SelectParam.newBuilder(inputEntry.getField(), inputEntry.getTitle()).setOptions(options)
				.setValue(inputEntry.getData()).setSize(SMALL).setPlaceHolder(inputEntry.getPlaceholder())
				.setEmit(Boolean.TRUE.equals(inputEntry.getIsEmit()) ? Collections.singletonList(CHANGE) : null)
				.build();
	}
	...
	private GroupParam getGroupParam(DqRuleInputEntry inputEntry) {
		return GroupParam.newBuilder(inputEntry.getField(), inputEntry.getTitle())
				.addValidate(Validate.newBuilder().setRequired(inputEntry.getIsValidate()).build())
