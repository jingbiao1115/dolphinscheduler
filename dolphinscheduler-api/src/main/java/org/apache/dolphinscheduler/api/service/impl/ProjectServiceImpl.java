...
public class ProjectServiceImpl extends BaseServiceImpl implements ProjectService {
	...
	public Result createProject(User loginUser, String name, String desc) {
		...
		try {
			project = Project.builder().name(name).code(CodeGenerateUtils.getInstance().genCode()).description(desc)
					.userId(loginUser.getId()).userName(loginUser.getUserName()).createTime(now).updateTime(now)
					.build();
	...
	public Map<String, Object> checkProjectAndAuth(User loginUser, Project project, long projectCode,
		...
		if (project == null) {
			...
			log.error("User does not have {} permission to operate project, userName:{}, projectCode:{}.", permission,
	...
	public Result queryProjectListPaging(User loginUser, Integer pageSize, Integer pageNo, String searchVal) {
		...
		IPage<Project> projectIPage = projectMapper.queryProjectListPaging(page, new ArrayList<>(projectIds),
				searchVal);
	...
	public Result queryProjectWithAuthorizedLevelListPaging(Integer userId, User loginUser, Integer pageSize,
		...
		IPage<Project> projectIPage = projectMapper.queryProjectListPaging(page, new ArrayList<>(allProjectIds),
				searchVal);
		...
		for (Project project : projectList) {
			if (userProjectIds.contains(project.getId())) {
				...
				if (projectUser == null) {
					// in this case, the user is the project owner, maybe it's better to set it to
					// ALL_PERMISSION.
					project.setPerm(Constants.DEFAULT_ADMIN_PERMISSION);
				} else {
	...
	public Result deleteProject(User loginUser, Long projectCode) {
		...
		List<ProcessDefinition> processDefinitionList = processDefinitionMapper
				.queryAllDefinitionList(project.getCode());
	...
	private Map<String, Object> getCheckResult(User loginUser, Project project, String perm) {
		Map<String, Object> checkResult = checkProjectAndAuth(loginUser, project,
				project == null ? 0L : project.getCode(), perm);
	 ...
	 * @param userId
	 *            user id
	 * @return projects which the user have permission to see, Except for items
	 ...
	 * @param project
	 *            project
	 * @return true if the user have permission to see the project, otherwise return
	 *         false
	 */
	...
	public Result queryAllProjectList(User user) {
		...
		List<Project> projects = projectMapper
				.queryAllProject(user.getUserType() == UserType.ADMIN_USER ? 0 : user.getId());
	...
	public Result queryAllProjectListForDependent() {
		Result result = new Result<>();
		List<Project> projects = projectMapper.queryAllProjectForDependent();
		result.setData(projects);
		putMsg(result, Status.SUCCESS);
		return result;
	}
	...
	public List<Long> getAuthorizedProjectCodes(User loginUser) {
		...
		return projectMapper.selectBatchIds(projectIds).stream().map(Project::getCode).collect(Collectors.toList());
	}
}
