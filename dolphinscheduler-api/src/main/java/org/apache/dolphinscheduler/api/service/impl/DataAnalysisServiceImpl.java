...
public class DataAnalysisServiceImpl extends BaseServiceImpl implements DataAnalysisService {
	...
	@Override
	public TaskInstanceCountVO getTaskInstanceStateCountByProject(User loginUser, Long projectCode, String startDate,
			String endDate) {
		...
		List<TaskInstanceStatusCountDto> taskInstanceStatusCounts = taskInstanceMapper
				.countTaskInstanceStateByProjectCodes(start, end, Lists.newArrayList(projectCode));
	...
	public TaskInstanceCountVO getAllTaskInstanceStateCount(User loginUser, String startDate, String endDate) {
		...
		Date end = endDate == null ? null : transformDate(endDate);
		List<TaskInstanceStatusCountDto> taskInstanceStatusCounts = taskInstanceMapper
				.countTaskInstanceStateByProjectCodes(start, end, projectCodes);
		return TaskInstanceCountVO.of(taskInstanceStatusCounts);
	}

	@Override
	public WorkflowInstanceCountVO getWorkflowInstanceStateCountByProject(User loginUser, Long projectCode,
	...
	public WorkflowInstanceCountVO getAllWorkflowInstanceStateCount(User loginUser, String startDate, String endDate) {
		...
		List<WorkflowInstanceStatusCountDto> workflowInstanceStatusCountDtos = processInstanceMapper
				.countWorkflowInstanceStateByProjectCodes(start, end, projectCodes);
		return WorkflowInstanceCountVO.of(workflowInstanceStatusCountDtos);
	}
	...
	public WorkflowDefinitionCountVO getWorkflowDefinitionCountByProject(User loginUser, Long projectCode) {
		projectService.checkProjectAndAuthThrowException(loginUser, projectCode, PROJECT_OVERVIEW);
		List<WorkflowDefinitionCountDto> workflowDefinitionCounts = processDefinitionMapper
				.countDefinitionByProjectCodes(Lists.newArrayList(projectCode));
	...
	public List<CommandStateCount> countCommandState(User loginUser) {
		...
		Map<CommandType, Integer> normalCountCommandCounts = commandMapper.countCommandState(null, null, projectCodes)
				.stream().collect(Collectors.toMap(CommandCount::getCommandType, CommandCount::getCount));

		// count error command state
		Map<CommandType, Integer> errorCommandCounts = errorCommandMapper.countCommandState(null, null, projectCodes)
				.stream().collect(Collectors.toMap(CommandCount::getCommandType, CommandCount::getCount));

		List<CommandStateCount> list = Arrays.stream(CommandType.values())
				.map(commandType -> new CommandStateCount(errorCommandCounts.getOrDefault(commandType, 0),
						normalCountCommandCounts.getOrDefault(commandType, 0), commandType))
	...
	public TaskCountDto countWorkflowStates(User loginUser, StatisticsStateRequest statisticsStateRequest) {
		...
		Date date = new Date();
		Date startTime = statisticsStateRequest.getStartTime() == null
		...
		List<ExecuteStatusCount> executeStatusCounts = processInstanceMapper.countInstanceStateV2(startTime, endTime,
				projectCode, workflowCode, model, projectIds);
	...
	public TaskCountDto countTaskStates(User loginUser, StatisticsStateRequest statisticsStateRequest) {
		...
		try {
			if (null == taskCode || null == workflowCode || null == projectCode) {
				projectCode = projectMapper.queryByName(projectName).getCode();
				workflowCode = processDefinitionMapper.queryByDefineName(projectCode, workflowName).getCode();
				// todo The comment can be canceled after repairing the duplicate taskname of
		...
		Optional<List<ExecuteStatusCount>> startTimeStates = Optional
				.ofNullable(taskInstanceMapper.countTaskInstanceStateByProjectIdsV2(startTime, endTime, projectIds));
		...
		List<ExecuteStatusCount> recounts = this.taskInstanceMapper
				.countTaskInstanceStateByProjectCodesAndStatesBySubmitTimeV2(startTime, endTime, projectCode,
						workflowCode, taskCode, model, projectIds, needRecountState);
	...
	public TaskCountDto countOneTaskStates(User loginUser, Long taskCode) {
		...
		Optional<List<ExecuteStatusCount>> startTimeStates = Optional
				.ofNullable(taskInstanceMapper.countTaskInstanceStateByProjectIdsV2(null, null, projectId));
		List<TaskExecutionStatus> needRecountState = setOptional(startTimeStates);
		if (needRecountState.size() == 0) {
			return new TaskCountDto(startTimeStates.get());
		}
		List<ExecuteStatusCount> recounts = this.taskInstanceMapper
				.countTaskInstanceStateByProjectCodesAndStatesBySubmitTimeV2(null, null, projectCode, null, taskCode,
						Constants.QUERY_ALL_ON_TASK, projectId, needRecountState);
		startTimeStates.orElseGet(ArrayList::new).addAll(recounts);
		List<ExecuteStatusCount> executeStatusCounts = startTimeStates.orElse(null);
		return new TaskCountDto(executeStatusCounts);
	}
	 ...
	 * <p>
	 * We only need projects which users have permission to see to determine whether
	 * the definition belongs to the user or not.
	 *
	 * @param loginUser
	 *            login user
	...
	public DefineUserDto countDefinitionByUserV2(User loginUser, Integer userId, Integer releaseState) {
		...
		List<Long> projectCodes = projectMapper.selectBatchIds(projectIds).stream().map(Project::getCode)
				.collect(Collectors.toList());

		List<WorkflowDefinitionCountDto> workflowDefinitionCountDtos = processDefinitionMapper
				.countDefinitionByProjectCodesV2(projectCodes, userId, releaseState);
	...
	private List<TaskExecutionStatus> setOptional(Optional<List<ExecuteStatusCount>> startTimeStates) {
		...
		if (startTimeStates.isPresent() && startTimeStates.get().size() != 0) {
			List<TaskExecutionStatus> instanceState = startTimeStates.get().stream().map(ExecuteStatusCount::getState)
					.collect(Collectors.toList());
