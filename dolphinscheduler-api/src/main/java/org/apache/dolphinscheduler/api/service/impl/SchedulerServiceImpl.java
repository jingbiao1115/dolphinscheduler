...
public class SchedulerServiceImpl extends BaseServiceImpl implements SchedulerService {
	...
	@Override
	@Transactional
	public Map<String, Object> insertSchedule(User loginUser, long projectCode, long processDefineCode, String schedule,
		...
		Schedule scheduleExists = scheduleMapper.queryByProcessDefinitionCode(processDefineCode);
		...
		log.info("Schedule create complete, projectCode:{}, processDefinitionCode:{}, scheduleId:{}.", projectCode,
				processDefineCode, scheduleObj.getId());
	...
	/**
	 * save schedule V2, will also change process definition's warningGroupId if
	 * schedule's warningGroupId be set
	 *
	 * @param loginUser
	 *            login user
	...
	public Schedule createSchedulesV2(User loginUser, ScheduleCreateRequest scheduleCreateRequest) {
		...
		ProcessDefinition processDefinition = processDefinitionMapper
				.queryByCode(scheduleCreateRequest.getProcessDefinitionCode());
		...
		Schedule scheduleExists = scheduleMapper
				.queryByProcessDefinitionCode(scheduleCreateRequest.getProcessDefinitionCode());
		if (scheduleExists != null) {
	...
	@Override
	@Transactional
	public Map<String, Object> updateSchedule(User loginUser, long projectCode, Integer id, String scheduleExpression,
			WarningType warningType, int warningGroupId, FailureStrategy failureStrategy,
			Priority processInstancePriority, String workerGroup, String tenantCode, Long environmentCode) {
		Map<String, Object> result = new HashMap<>();
	 ...
	 *            the schedule object will be updated
	...
	public Result querySchedule(User loginUser, long projectCode, long processDefineCode, String searchVal,
		...
		IPage<Schedule> schedulePage = scheduleMapper.queryByProjectAndProcessDefineCodePaging(page, projectCode,
		...
		int total = (int) schedulePage.getTotal();
	...
	public PageInfo<Schedule> filterSchedules(User loginUser, ScheduleFilterRequest scheduleFilterRequest) {
		...
		int pageSize = scheduleFilterRequest.getPageSize();
		int total = (int) scheduleIPage.getTotal();
		PageInfo<Schedule> pageInfo = new PageInfo<>(scheduleFilterRequest.getPageNo(), pageSize);
		pageInfo.setTotal(total);
		pageInfo.setTotalPage((int) Math.ceil((double) total / pageSize));
	...
	public Map<String, Object> previewSchedule(User loginUser, String schedule) {
		...
		List<ZonedDateTime> selfFireDateList = CronUtils.getSelfFireDateList(startTime, endTime, cron,
				Constants.PREVIEW_SCHEDULE_EXECUTE_COUNT);
		List<String> previewDateList = selfFireDateList.stream().map(t -> DateUtils.dateToString(t, zoneId))
	 ...
	 */
	@Override
	public Map<String, Object> updateScheduleByProcessDefinitionCode(User loginUser, long projectCode,
			long processDefinitionCode, String scheduleExpression, WarningType warningType, int warningGroupId,
			FailureStrategy failureStrategy, Priority processInstancePriority, String workerGroup, String tenantCode,
	...
	private void updateSchedule(Map<String, Object> result, Schedule schedule, ProcessDefinition processDefinition,
			String scheduleExpression, WarningType warningType, int warningGroupId, FailureStrategy failureStrategy,
			Priority processInstancePriority, String workerGroup, String tenantCode, long environmentCode) {
