...
public class PythonGateway {
	...
	// We use admin user's user_id to skip some permission issue from python gateway
	// service
	...
	// TODO replace this user to build in admin user if we make sure build in one
	// could not be change
	...
	// TODO Should we import package in python client side? utils package can but
	// service can not, why
	// Core api
	...
	public Map<String, Long> getCodeAndVersion(String projectName, String processDefinitionName, String taskName)
			throws CodeGenerateUtils.CodeGenerateException {
		Project project = projectMapper.queryByName(projectName);
		Map<String, Long> result = new HashMap<>();
		// project do not exists, mean task not exists too, so we should directly return
		// init value
		if (project == null) {
		...
		ProcessDefinition processDefinition = processDefinitionMapper.queryByDefineName(project.getCode(),
				processDefinitionName);
		// In the case project exists, but current workflow still not created, we should
		// also return the init
		// version of it
		...
		TaskDefinition taskDefinition = taskDefinitionMapper.queryByName(project.getCode(), processDefinition.getCode(),
				taskName);
	...
	/**
	 * create or update workflow. If workflow do not exists in Project=`projectCode`
	 * would create a new one If workflow already exists in Project=`projectCode`
	 * would update it
	 *
	 * @param userName
	 *            user name who create or update workflow
	 * @param projectName
	 *            project name which workflow belongs to
	 * @param name
	 *            workflow name
	 * @param description
	 *            description
	 ...
	 *            global params
	 * @param schedule
	 *            schedule for workflow, will not set schedule if null, and if would
	 *            always fresh exists schedule if not null
	 * @param onlineSchedule
	 *            Whether set the workflow's schedule to online state
	 * @param warningType
	 *            warning type
	 * @param warningGroupId
	 *            warning group id
	 * @param timeout
	 *            timeout for workflow working, if running time longer than timeout,
	 *            task will mark as fail
	 * @param workerGroup
	 *            run task in which worker group
	 * @param taskRelationJson
	 *            relation json for nodes
	 * @param taskDefinitionJson
	 *            taskDefinitionJson
	 * @param otherParamsJson
	 *            otherParamsJson handle other params
	 * @return create result code
	 */
	public Long createOrUpdateWorkflow(String userName, String projectName, String name, String description,
			String globalParams, String schedule, boolean onlineSchedule, String warningType, int warningGroupId,
			int timeout, String workerGroup, int releaseState, String taskRelationJson, String taskDefinitionJson,
		...
		if (processDefinition != null) {
			...
			processDefinitionService.updateProcessDefinition(user, projectCode, name, processDefinitionCode,
					description, globalParams, null, timeout, taskRelationJson, taskDefinitionJson, executionTypeEnum);
		} else {
			Map<String, Object> result = processDefinitionService.createProcessDefinition(user, projectCode, name,
					description, globalParams, null, timeout, taskRelationJson, taskDefinitionJson, otherParamsJson,
					executionTypeEnum);
	 ...
	 *
	 * @param user
	 *            user who create or update schedule
	 * @param projectCode
	 *            project which workflow belongs to
	 * @param workflowName
	 *            workflow name
	 */
	private ProcessDefinition getWorkflow(User user, long projectCode, String workflowName) {
		Map<String, Object> verifyProcessDefinitionExists = processDefinitionService.verifyProcessDefinitionName(user,
				projectCode, workflowName, 0);
	...
	/**
	 * create or update workflow schedule. It would always use latest schedule
	 * define in workflow-as-code, and set schedule online when it's not null
	 *
	 * @param user
	 *            user who create or update schedule
	 * @param projectCode
	 ...
	 *            warning group id
	 */
	private void createOrUpdateSchedule(User user, long projectCode, long workflowCode, String schedule,
			boolean onlineSchedule, String workerGroup, String warningType, int warningGroupId) {
		...
		if (scheduleObj == null) {
			...
			Map<String, Object> result = schedulerService.insertSchedule(user, projectCode, workflowCode, schedule,
					WarningType.valueOf(warningType), warningGroupId, DEFAULT_FAILURE_STRATEGY, DEFAULT_PRIORITY,
					workerGroup, user.getTenantCode(), DEFAULT_ENVIRONMENT_CODE);
	...
	public void execWorkflowInstance(String userName, String projectName, String workflowName, String cronTime,
			String workerGroup, String warningType, Integer warningGroupId, Integer timeout) {
		User user = usersService.queryUser(userName);
		Project project = projectMapper.queryByName(projectName);
		ProcessDefinition processDefinition = processDefinitionMapper.queryByDefineName(project.getCode(),
				workflowName);
		...
		executorService.execProcessInstance(user, project.getCode(), processDefinition.getCode(), cronTime, null,
				DEFAULT_FAILURE_STRATEGY, null, DEFAULT_TASK_DEPEND_TYPE, WarningType.valueOf(warningType),
				warningGroupId, DEFAULT_RUN_MODE, DEFAULT_PRIORITY, workerGroup, user.getTenantCode(),
				DEFAULT_ENVIRONMENT_CODE, timeout, null, null, DEFAULT_DRY_RUN, DEFAULT_TEST_FLAG,
				COMPLEMENT_DEPENDENT_MODE, processDefinition.getVersion(), false, DEFAULT_EXECUTION_ORDER);
	}

	// side object
	/*
	 * Grant project's permission to user. Use when project's created user not
	 * current but Python API use it to change workflow.
	 */
	...
	/*
	 * Grant or create project. Create a new project if project do not exists, and
	 * grant the project permission to user if project exists but without permission
	 * to this user.
	 */
	...
	public User createUser(String userName, String userPassword, String email, String phone, String tenantCode,
	...
	/**
	 * Get single datasource by given datasource name. if type is not null, it will
	 * return the datasource match the type.
	 *
	...
	/**
	 * Get workflow object by given workflow name. It returns map contain workflow
	 * id, name, code. Useful in Python API create subProcess task which need
	...
	/**
	 * Get project, workflow, task code. Useful in Python API create dependent task
	 * which need workflow information.
	 *
	 * @param projectName
	 *            project name which workflow belongs to
	 * @param workflowName
	...
	public Map<String, Object> getDependentInfo(String projectName, String workflowName, String taskName) {
		...
		ProcessDefinition processDefinition = processDefinitionMapper.queryByDefineName(projectCode, workflowName);
		if (processDefinition == null) {
			String msg = String.format("Can not find valid workflow by name %s", workflowName);
			log.error(msg);
			throw new IllegalArgumentException(msg);
		...
		if (taskName != null) {
			TaskDefinition taskDefinition = taskDefinitionMapper.queryByName(projectCode, processDefinition.getCode(),
					taskName);
	...
	/**
	 * Get resource by given program type and full name. It returns map contain
	 * resource id, name. Useful in Python API create flink or spark task which need
	 * workflow information.
	 *
	 * @param programType
	 *            program type one of SCALA, JAVA and PYTHON
	 * @param fullName
	...
	public Map<String, Object> getResourcesFileInfo(String programType, String fullName) {
		...
		List<ResourceComponent> namedResources = resourcesComponent.stream()
				.filter(s -> fullName.equals(s.getFullName())).collect(Collectors.toList());
		if (CollectionUtils.isEmpty(namedResources)) {
			String msg = String.format("Can not find valid resource by program type %s and name %s", programType,
					fullName);
			log.error(msg);
			throw new IllegalArgumentException(msg);
		}
	...
	/**
	 * Get resource by given resource type and full name. It return map contain
	 * resource id, name. Useful in Python API create task which need workflow
	 * information.
	 *
	 * @param userName
	 *            user who query resource
	 * @param fullName
	 *            full name of the resource
	 * @return StorageEntity object which contains necessary information about
	 *         resource
	 */
	...
	/**
	 * create or update resource. If the folder is not already created, it will be
	 *
	 * @param userName
	 *            user who create or update resource
	 * @param fullName
	 *            The fullname of resource.Includes path and suffix.
	 * @param resourceContent
	 *            content of resource
	 * @return StorageEntity object which contains necessary information about
	 *         resource
	 */
	public StorageEntity createOrUpdateResource(String userName, String fullName, String resourceContent)
			throws Exception {
	...
	private void start() {
		try {
			...
			GatewayServerBuilder serverBuilder = new GatewayServer.GatewayServerBuilder().entryPoint(this)
					.javaAddress(gatewayHost).javaPort(pythonGatewayConfiguration.getGatewayServerPort())
