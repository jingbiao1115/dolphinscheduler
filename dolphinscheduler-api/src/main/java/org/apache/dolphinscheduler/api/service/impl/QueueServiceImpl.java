...
public class QueueServiceImpl extends BaseServiceImpl implements QueueService {
	 ...
	 *
	 * @param queue
	 *            The queue object want to create
	 */
	 ...
	 *
	 * @param existsQueue
	 *            The exists queue object
	 * @param updateQueue
	 *            The queue object want to update
	 */
	private void updateQueueValid(Queue existsQueue, Queue updateQueue) throws ServiceException {
		// Check the exists queue and the necessary of update operation, in not exist
		// checker have to use updateQueue to
		// avoid NPE
	...
	public PageInfo<Queue> queryList(User loginUser, String searchVal, Integer pageNo, Integer pageSize) {
		...
		pageInfo.setTotalPage((int) Math.ceil((double) count / pageSize));
	...
	public Queue updateQueue(User loginUser, int id, String queue, String queueName) {
		...
		if (checkIfQueueIsInUsing(existsQueue.getQueueName(), updateQueue.getQueueName())) {
			...
			Integer relatedUserNums = userMapper.updateUserQueue(existsQueue.getQueueName(),
					updateQueue.getQueueName());
	...
	/**
	 * check queue exist if exists return true，not exists return false check queue
	 * exist
	 *
	 * @param queue
	 *            queue
	...
	/**
	 * check queue name exist if exists return true，not exists return false
	 *
	 * @param queueName
	 *            queue name
	 * @return true if the queue name not exists, otherwise return false
	...
	/**
	 * check old queue name using by any user if need to update user
	 *
	 * @param oldQueue
	 *            old queue name
	 * @param newQueue
