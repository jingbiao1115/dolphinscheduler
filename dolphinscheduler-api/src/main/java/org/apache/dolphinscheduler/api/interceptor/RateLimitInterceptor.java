...
public class RateLimitInterceptor implements HandlerInterceptor {
	...
	private LoadingCache<String, RateLimiter> tenantRateLimiterCache = CacheBuilder.newBuilder().maximumSize(100)
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if (trafficConfiguration.isGlobalSwitch()) {
			if (!globalRateLimiter.tryAcquire()) {
				response.setStatus(HttpStatus.TOO_MANY_REQUESTS.value());
				log.warn("Too many request, reach global rate limit, current qps is {}", globalRateLimiter.getRate());
				return false;
			}
		}
		return true;
	...
	public RateLimitInterceptor(ApiConfig.TrafficConfiguration trafficConfiguration) {
		...
		if (trafficConfiguration.isGlobalSwitch()) {
			this.globalRateLimiter = RateLimiter.create(trafficConfiguration.getMaxGlobalQpsRate(), 1,
					TimeUnit.SECONDS);
		}
	}
