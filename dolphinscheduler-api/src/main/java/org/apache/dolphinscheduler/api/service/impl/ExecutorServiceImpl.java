...
public class ExecutorServiceImpl extends BaseServiceImpl implements ExecutorService {
	 ...
	 *            timeout
	 * @param startParams
	 *            the global param values which pass to new process instance
	 * @param expectedParallelismNumber
	 *            the expected parallelism number when execute complement in
	 *            parallel mode
	 * @param testFlag
	 *            testFlag
	 * @param executionOrder
	 *            the execution order when complementing data
	 * @return execute process instance code
	 */
	@Override
	@Transactional(rollbackFor = Exception.class)
	public Map<String, Object> execProcessInstance(User loginUser, long projectCode, long processDefinitionCode,
			String cronTime, CommandType commandType, FailureStrategy failureStrategy, String startNodeList,
			TaskDependType taskDependType, WarningType warningType, Integer warningGroupId, RunMode runMode,
			Priority processInstancePriority, String workerGroup, String tenantCode, Long environmentCode,
			Integer timeout, Map<String, String> startParams, Integer expectedParallelismNumber, int dryRun,
			int testFlag, ComplementDependentMode complementDependentMode, Integer version, boolean allLevelDependent,
		...
		int create = this.createCommand(triggerCode, commandType, processDefinition.getCode(), taskDependType,
				failureStrategy, startNodeList, cronTime, warningType, loginUser.getId(), warningGroupId, runMode,
				processInstancePriority, workerGroup, tenantCode, environmentCode, startParams,
				expectedParallelismNumber, dryRun, testFlag, complementDependentMode, allLevelDependent,
				executionOrder);

		if (create > 0) {
			processDefinition.setWarningGroupId(warningGroupId);
			processDefinitionMapper.updateById(processDefinition);
			log.info("Create command complete, processDefinitionCode:{}, commandCount:{}.", processDefinition.getCode(),
					create);
	...
	public boolean checkSubProcessDefinitionValid(ProcessDefinition processDefinition) {
		...
		taskDefinitions.stream()
				...
				.forEach(taskDefinition -> processDefinitionCodeSet.add(Long.valueOf(
						JSONUtils.getNodeString(taskDefinition.getTaskParams(), CMD_PARAM_SUB_PROCESS_DEFINE_CODE))));
	...
	public Map<String, Object> execute(User loginUser, long projectCode, Integer processInstanceId,
		...
		executeClient.executeWorkflowInstance(
				new ExecuteContext(workflowInstance, processDefinition, loginUser, executeType));
	...
	public WorkflowExecuteResponse executeTask(User loginUser, long projectCode, Integer processInstanceId,
		...
		if (!commandService.verifyIsNeedCreateCommand(command)) {
			...
			putMsg(response, Status.PROCESS_INSTANCE_EXECUTING_COMMAND, String.valueOf(processDefinition.getCode()));
			return response;
		}
		...
		if (create > 0) {
			...
			log.error(
					...
					command.getCommandType().getDescp(), command.getProcessDefinitionCode(),
					processDefinition.getVersion(), processInstanceId);
				...
				for (ProcessDefinition processDefinitionTmp : processDefinitionList) {
					...
					if (processDefinitionTmp.getReleaseState() != ReleaseState.ONLINE) {
						log.warn("Subprocess definition {} of process definition {} is not {}.",
								processDefinitionTmp.getName(), processDefinition.getName(),
	...
	private int createCommand(Long triggerCode, CommandType commandType, long processDefineCode, TaskDependType nodeDep,
			...
			int executorId, Integer warningGroupId, RunMode runMode, Priority processInstancePriority,
			String workerGroup, String tenantCode, Long environmentCode, Map<String, String> startParams,
			Integer expectedParallelismNumber, int dryRun, int testFlag,
		...
		if (commandType == CommandType.COMPLEMENT_DATA) {
			...
			try {
				log.info("Start to create {} command, processDefinitionCode:{}.", command.getCommandType().getDescp(),
						processDefineCode);
	...
	private int createComplementCommand(Long triggerCode, Command command, Map<String, String> cmdParam,
			List<ZonedDateTime> dateTimeList, List<Schedule> schedules, ComplementDependentMode complementDependentMode,
			boolean allLevelDependent) {

		String dateTimeListStr = dateTimeList.stream().map(item -> DateUtils.dateToString(item))
		...
		if (createCount > 0) {
			log.info("Create {} command complete, processDefinitionCode:{}", command.getCommandType().getDescp(),
					command.getProcessDefinitionCode());
		} else {
			log.error("Create {} command error, processDefinitionCode:{}", command.getCommandType().getDescp(),
					command.getProcessDefinitionCode());
		}
	...
	/**
	 * create complement command close left and close right
	 *
	...
	protected int createComplementCommandList(Long triggerCode, String scheduleTimeParam, RunMode runMode,
			Command command, Integer expectedParallelismNumber, ComplementDependentMode complementDependentMode,
		...
		if (scheduleParam.containsKey(CMD_PARAM_COMPLEMENT_DATA_START_DATE)
			...
			if (startDate != null && endDate != null) {
				listDate = CronUtils.getSelfFireDateList(DateUtils.stringToZoneDateTime(startDate),
		...
		if (scheduleParam.containsKey(CMD_PARAM_COMPLEMENT_DATA_SCHEDULE_DATE_LIST)) {
			...
			if (StringUtils.isNotBlank(dateList)) {
				listDate = Splitter.on(COMMA).splitToStream(dateList)
						.map(item -> DateUtils.stringToZoneDateTime(item.trim())).distinct()
						.collect(Collectors.toList());
			}
		}
	...
	public int createComplementDependentCommand(List<Schedule> schedules, Command command, boolean allLevelDependent) {
		...
		List<DependentProcessDefinition> dependentProcessDefinitionList = getComplementDependentDefinitionList(
				dependentCommand.getProcessDefinitionCode(), CronUtils.getMaxCycle(schedules.get(0).getCrontab()),
				dependentCommand.getWorkerGroup(), allLevelDependent);
	...
	private List<DependentProcessDefinition> getComplementDependentDefinitionList(long processDefinitionCode,
			CycleEnum processDefinitionCycle, String workerGroup, boolean allLevelDependent) {
		List<DependentProcessDefinition> dependentProcessDefinitionList = checkDependentProcessDefinitionValid(
				...
				processDefinitionCycle, workerGroup, processDefinitionCode);
		...
		if (allLevelDependent) {
			...
			while (true) {
				List<DependentProcessDefinition> childDependentList = childList.stream()
	 ...
	 * the schedule cycle of the dependent process definition and if there is no
	 * worker group in the schedule, use the complement selection's worker group
	 */
	private List<DependentProcessDefinition> checkDependentProcessDefinitionValid(
			...
			String workerGroup, long upstreamProcessDefinitionCode) {
	...
	public void execStreamTaskInstance(User loginUser, long projectCode, long taskDefinitionCode,
			int taskDefinitionVersion, int warningGroupId, String workerGroup, String tenantCode, Long environmentCode,
