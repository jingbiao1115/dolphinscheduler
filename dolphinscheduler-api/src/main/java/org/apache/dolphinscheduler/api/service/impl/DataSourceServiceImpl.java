...
public class DataSourceServiceImpl extends BaseServiceImpl implements DataSourceService {
	...
	public BaseDataSourceParamDTO queryDataSource(int id, User loginUser) {
		...
		// type
		BaseDataSourceParamDTO baseDataSourceParamDTO = DataSourceUtils.buildDatasourceParamDTO(dataSource.getType(),
	...
	public PageInfo<DataSource> queryDataSourceListPaging(User loginUser, String searchVal, Integer pageNo,
		...
		int total = (int) (dataSourceList != null ? dataSourceList.getTotal() : 0L);
		// pageInfo.setTotal((int) (dataSourceList != null ? dataSourceList.getTotal() :
		// 0L));
	...
	public List<ParamsOptions> getTables(Integer datasourceId, String database) {
		...
		BaseConnectionParam connectionParam = (BaseConnectionParam) DataSourceUtils
				.buildConnectionParams(dataSource.getType(), dataSource.getConnectionParams());
		...
		try {
			...
			tables = metaData.getTables(database, getDbSchemaPattern(dataSource.getType(), schema, connectionParam),
					"%", TABLE_TYPES);
			if (null == tables) {
