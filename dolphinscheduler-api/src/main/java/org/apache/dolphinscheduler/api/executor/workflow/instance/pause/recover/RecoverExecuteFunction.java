...
public class RecoverExecuteFunction implements ExecuteFunction<RecoverExecuteRequest, RecoverExecuteResult> {
	...
	public RecoverExecuteResult execute(RecoverExecuteRequest request) throws ExecuteRuntimeException {
		...
		if (!workflowInstance.getState().isPause()) {
			throw new ExecuteRuntimeException(String.format("The workflow instance: %s state is %s, cannot recovery",
		...
		Command command = Command.builder().commandType(CommandType.RECOVER_SUSPENDED_PROCESS)
				...
				.executorId(request.getExecuteUser().getId()).testFlag(workflowInstance.getTestFlag()).build();
	...
	private Map<String, Object> createCommandParam(ProcessInstance workflowInstance) {
		return new ImmutableMap.Builder<String, Object>()
				.put(CMD_PARAM_RECOVER_PROCESS_ID_STRING, workflowInstance.getId()).build();
	}
