...
public class JdbcOperator {
	...
	public Long insertOrUpdateEphemeralData(String key, String value) throws SQLException {
		...
		jdbcRegistryData = JdbcRegistryData.builder().dataKey(key).dataValue(value)
				.dataType(DataType.EPHEMERAL.getTypeValue()).lastTerm(System.currentTimeMillis()).build();
	...
	public long insertOrUpdatePersistentData(String key, String value) throws SQLException {
		...
		jdbcRegistryData = JdbcRegistryData.builder().dataKey(key).dataValue(value)
				.dataType(DataType.PERSISTENT.getTypeValue()).lastTerm(System.currentTimeMillis()).build();
		jdbcRegistryDataMapper.insert(jdbcRegistryData);
		return jdbcRegistryData.getId();
	}
	...
	public List<String> getChildren(String key) throws SQLException {
		return jdbcRegistryDataMapper.fuzzyQueryByKey(key).stream().map(JdbcRegistryData::getDataKey)
	...
	public JdbcRegistryLock tryToAcquireLock(String key) throws SQLException {
		JdbcRegistryLock jdbcRegistryLock = JdbcRegistryLock.builder().lockKey(key)
