...
public class RegistryClient {
		...
		for (Map.Entry<String, String> entry : serverMaps.entrySet()) {
			...
			switch (registryNodeType) {
				...
				case WORKER :
					...
					break;
				case ALERT_SERVER :
					AlertServerHeartBeat alertServerHeartBeat = JSONUtils.parseObject(heartBeatJson,
							AlertServerHeartBeat.class);
			...
			// todo: add host, port in heartBeat Info, so that we don't need to parse this
			// again
	...
	public boolean checkNodeExists(String host, RegistryNodeType nodeType) {
		return getServerMaps(nodeType).keySet().stream().anyMatch(it -> it.contains(host));
	}
