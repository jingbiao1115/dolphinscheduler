...
/**
 * The SparkRuntimeEnvironment is responsible for creating SparkSession and
 * SparkExecution
 */
public class SparkRuntimeEnvironment {
	...
	public void prepare(boolean hiveClientSupport) {
		...
		this.sparkSession = hiveClientSupport
				? sparkSessionBuilder.enableHiveSupport().getOrCreate()
				: sparkSessionBuilder.getOrCreate();
	}

	private SparkConf createSparkConf() {
		SparkConf conf = new SparkConf();
		this.config.entrySet().forEach(entry -> conf.set(entry.getKey(), String.valueOf(entry.getValue())));
